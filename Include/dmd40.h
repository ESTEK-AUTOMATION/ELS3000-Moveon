...
#define DMD_CMD_ASSIGN_READ_SLOT                 231         /* Gives slot address for RTV read by UltimET */
#define DMD_CMD_ASSIGN_WRITE_SLOT                232         /* Sets slot address for RTV written by UltimET */
...
#define DMD_CMD_FCOFF                            301         /* Reset dual axis Force Control */
#define DMD_CMD_FCON                             300         /* Set dual axis Force Control */
...
#define DMD_PPK_ERROR_DOUT_MASK          357         /* DOUT mask for security management */
...
#define DMD_PPK_PROFILE_LIMIT_MODE       36          /* Enables software position limits (KL34, KL35) */
#define DMD_PPK_PROFILE_TYPE             202         /* Movement type */
#define DMD_KL_PROFILE_VELOCITY         211         /* Absolute max speed */
#define DMD_KF_SOFTWARE_CURRENT_LIMIT   60          /* Software current limit */
...
#define DMD_PPK_TRIG_CONT_SETTING        337         /* Trigger continuous setting */
...
#define DMD_MON_ERROR_DOUT_MASK          357         /* Actual DOUT mask for security management */
...
#define DMD_MON_TRIG_FIRST_EVENT_INDEX   350         /* First event free */
#define DMD_MON_TRIG_NB_FREE_EVENT       349         /* Number of free event */
...
#define DMD_MON_TRIG_TREATED_EVT_PER_GRP 351         /* Number of trigger per group */
...
int	    _DMD_EXPORT dmd_get_fw_edi_block_list(int product, int hw_version, dword fw_version, int max_block, DMD_FW_BLOCK blocks[], int *nb_blocks);
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum {SEQUENCE_ERROR_0409 = 409 };             
    enum {SEQUENCE_ERROR_0410 = 410 };             
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum { ASSIGN_READ_SLOT = 231 };                 /* Gives slot address for RTV read by UltimET */
    enum { ASSIGN_WRITE_SLOT = 232 };                /* Sets slot address for RTV written by UltimET */
    ...
    enum { FCOFF = 301 };                            /* Reset dual axis Force Control */
    enum { FCON = 300 };                             /* Set dual axis Force Control */
    enum { FREE_SLOT = 531 };                        /* Free TransnET slot */
    enum { GET_FILE_SIZE = 228 };                    /* Get file's size on target */
    enum { GET_SLOT = 530 };                         /* Get a free TransnET slot */
    ...
    enum { MVETILT = 480 };                          /* ZxT Starts movement on Rx, Ry Axis */
    enum { NEW_FC = 305 };                           /* New Force Control */
    enum { OR_NOT_REGISTER = 98 };                   /* OR NOT register */
    enum { OR_REGISTER = 96 };                       /* OR register */
    enum { PARAMETER_SWITCH = 1536 };                /* Parameter switch between active & inactive set */
    enum { PROFILED_MOVE = 60 };                     /* Start movement */
    enum { PROFILED_MOVE_NDOF = 1539 };              /* Move with several DOF */
    ...
    enum { RESET_FC = 304 };                         /* Reset single axis Force Control */
    enum { RIC = 753 };                              /* Reset IO update cycle count */
    enum { RMVETILT = 481 };                         /* ZxT Starts relative movement on Rx, Ry Axis */
    ...
    enum { SET_FC = 303 };                           /* Set single axis Force Control */
    enum { SET_LOCAL_TIME = 27 };                    /* Set Local Time */
    ...
    enum { SET_VECTOR_PARAM = 1537 };                /* Set a vector parameter */
    enum { SET_VECTOR_PARAM_MASK = 1538 };           /* Set a vector parameter with mask */
    ...
    enum { SETTILT = 483 };                          /* ZxT Starts polar movement on Rx, Ry */
    ...
    enum { SPI_CONFIG_BURST = 586 };                 /* Configures SPI burst transfer */
    enum { SPI_CONFIG_CYCLIC = 585 };                /* Configures SPI cyclic transfer */
    enum { SPI_CONNECT = 580 };                      /* Connects and configures spi */
    enum { SPI_DISCONNECT = 581 };                   /* Disconnects spi */
    enum { SPI_READ = 582 };                         /* Spi read */
    enum { SPI_READ_WRITE = 584 };                   /* Spi write and read */
    enum { SPI_RST = 589 };                          /* Waits end of burst transfer */
    enum { SPI_WRITE = 583 };                        /* Spi write */
    enum { START_BURST = 587 };                      /* Starts burst transfer */
    enum { START_CSM = 224 };                        /* Starts a customer software module */
    enum { START_CYCLIC = 590 };                     /* Starts UA cyclic task */
    ...
    enum { STETILT = 486 };                          /* ZxT Adds a step to the reference position */
    enum { STOP_CSM = 225 };                         /* Stops a customer software module */
    enum { STOP_CYCLIC = 591 };                      /* Stops UA cyclic task */
    ...
    enum { WAIT_BURST = 588 };                       /* Waits end of burst transfer */
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum { DISPLAY_MODE = 66 };                      /* ComET's display mode */
    ...
    enum { ERROR_DOUT_MASK = 357 };                  /* DOUT mask for security management */
    ...
    enum { HOME_OFFSET = 45 };                       /* Homing offset on absolute position */
    enum { HOMING_ACCELERATION = 42 };               /* Homing acceleration */
    enum { HOMING_CURRENT_LIMIT = 44 };              /* Homing current limit for mech end stop detection */
    ...
    enum { INIT_PULSE_LEVEL = 91 };                  /* Phasing pulse current level */
    ...
    enum { MOTION_AND_CONSIGNE = 78 };               /* Position Controller Mode */
    ...
    enum { MOTOR_POLE_NB = 54 };                     /* Motor number of pole pairs */
    ...
    enum { PROFILE_LIMIT_MODE = 36 };                /* Enables software position limits (KL34, KL35) */
    enum { PROFILE_TYPE = 202 };                     /* Movement type */
    enum { PROFILE_VELOCITY = 211 };                 /* Absolute max speed */
    enum { SOFTWARE_CURRENT_LIMIT = 60 };            /* Software current limit */
    ...
    enum { TRIG_CONT_SETTING = 337 };                /* Trigger continuous setting */
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum { CL_ACTUAL_VALUE = 31 };                   /* Real current Iq measured */
    ...
    enum { CL_DEMAND_VALUE = 30 };                   /* Theoretical current Iq for current loop */
    enum { CL_DEMAND_VALUE_LOGICAL = 249 };          /* Theoretical current Iq for current loop */
    ...
    enum { ERROR_DOUT_MASK = 357 };                  /* Actual DOUT mask for security management */
    ...
    enum { TRIG_FIRST_EVENT_INDEX = 350 };           /* First event free */
    enum { TRIG_NB_FREE_EVENT = 349 };               /* Number of free event */
    ...
    enum { TRIG_TREATED_EVT_PER_GRP = 351 };         /* Number of trigger per group */
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum { BIT11_ENCODER = 62 };                  /* Analog encoder signal amplitude in volt (11 bit) */
    enum { BIT11P2 = 82 };                        /* Analog encoder (11 bit) */
    enum { BIT15 = 65 };                          /* 2^15 = 32768 correspond to 1.0 */
    enum { BIT15_ENCODER = 63 };                  /* Analog encoder signal amplitude in volt (15 bit) */
    ...
    enum { ISO_POS = 70 };                        /* Increment register in ISO (position) (m or t 1:1) */
    ...
    enum { MLTI = 30 };                           /* manager loop time increment (mlti) */
