...
int     _ETB_EXPORT etb_is_open(ETB *etb, ebool *open);
...
int		_ETB_EXPORT etb_rtv_watchdog(ETB *etb, int rtv_id, int watchdog);
...
int		_ETB_EXPORT etb_cyclic_watchdog(ETB *etb, ETB_CYCLIC_TYPES cyclic_type, int watchdog);
...
void	_ETB_EXPORT etb_shm_reserved_function1(ETB *etb);
int 	_ETB_EXPORT etb_shm_reserved_function2(ETB *etb, ETB_ETCOM *rec, eint64 mask);
int 	_ETB_EXPORT etb_shm_reserved_function3(ETB *etb, ETB_ETCOM *rec, eint64 *mask);
int 	_ETB_EXPORT etb_shm_reserved_function4(ETB *etb, char *url);
int 	_ETB_EXPORT etb_shm_reserved_function5(ETB *etb, int par1, int par2);
 ...
 *-----------------------------------------------------------------------------*/
	...
	public:
		...
		void rtv_watchdog(int rtvId, int watchdog) {
			ERRCHK(etb_rtv_watchdog(etb, rtvId, watchdog));
		}
		void cyclic_watchdog(int rtvId, ETB_CYCLIC_TYPES cyclicType, int watchdog) {
			ERRCHK(etb_cyclic_watchdog(etb, cyclicType, watchdog));
		}
		void clearWatchdog() {
...
#ifdef ETB_OO_API
typedef void (ETB_CALLBACK *EtbIsoConverter)(EtbBus bus, EtbEtcomValue *incr, double *iso, int incr_type, int conv, int axis, ebool to_iso, void *user);
#endif /* ETB_OO_API */
