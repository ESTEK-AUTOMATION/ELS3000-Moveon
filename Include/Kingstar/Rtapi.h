...
typedef enum _ERTSTATE
{
	RT_STATE_READY = 0x00,
	RT_STATE_WAIT = 0x01,
	RT_STATE_SUSPENDED = 0x02,
	RT_STATE_SLEEP = 0x03,
	RT_STATE_FROZEN = 0x04,
	RT_STATE_DEBUGGED = 0x05,
} ERTSTATE;

// This structure defines process info retrieved by RtGetProcessInfoByPID 
typedef struct _RT_PROCESS_INFO
{
	DWORD Size; // Structure size
	DWORD State;
} RT_PROCESS_INFO, *PRT_PROCESS_INFO;
...
// Type mspace is an opaque type representing an MSpace, which is an independent region of memory that
// supports external or internal allocation per RTSS process.  See the Local Memory documentation for details
// about external and internal MSpaces.
#if defined ( _AMD64_ )
    typedef void* mspace;
#else
    typedef ULONG64 mspace;
#endif

// This function gets mspace descriptors of external and internal allocation space 
// for the specified RTSS process or RTSS system process.
BOOL RTAPI RtGetProcessMSpace(HANDLE hProcess, mspace *ExtMSpace, mspace *IntMSpace);

// This struct is used by RtQueryProcessMSpace.
// It contains various summary statistics of the local pool and pool cache within one memory allocation space (MSpace).
typedef struct _MSPACE_INFO {
    ULONG64 MSpacePeakUsage;           // MSpace peak (statistical maximum) usage
    ULONG64 MSpaceSize;                // Total memory in MSpace
    ULONG64 MSpaceFree;                // Total free memory in MSpace
    ULONG64 MSpaceUsed;                // Total used memory in MSpace
    ULONG64 MaxContigFreeMem;          // Maximum virtually contiguous free memory in MSpace
    ULONG64 NumFreeChunksInCache;      // Number of free chunks in Pool Cache
    ULONG64 CacheFree;                 // Free memory in Pool Cache
    ULONG64 CacheUsed;                 // Used memory in Pool Cache
    ULONG64 NumFreeRangesInPool;       // Number of free ranges in Local Pool
    ULONG64 PoolFree;                  // Free memory in Local Pool
    ULONG64 PoolUsed;                  // Used memory in Local Pool
    ULONG64 AutoExpandSize;            // Auto expand MSpace size
    BOOL    AutoExpandEnabled;         // Auto expand MSpace is enabled
} MSPACE_INFO, *PMSPACE_INFO;

// This enum is used by RtQueryProcessMSpace.
// It contains the performance level for the call of RtQueryProcessMSpace.
typedef enum _PERFORMANCE_LEVEL {
    LowestPerformanceLevel = 0,    //Fill in and print MSPACE_INFO structure and fragmentations
    LowPerformanceLevel,           //Fill in and print MSPACE_INFO structure
    NormalPerformanceLevel,        //Fill in MSPACE_INFO structure
    HighPerformanceLevel,          //Fill in some fields of MSPACE_INFO structure with existing trace values, zero'ed those unavailable
    PerformanceLevelUndefined
} PERFORMANCE_LEVEL, *PPERFORMANCE_LEVEL;

// This function queries allocation space information in the specified MSpace of process,
// including various summary statistics of the pool cache, and the local pool size as a whole, 
// the free memory size available, and the maximum virtually contiguous free memory size.
//   NOTE: if MSpace is NULL, default to the external allocation space of current process
BOOL    RTAPI RtQueryProcessMSpace(mspace MSpace, MSPACE_INFO *MSpaceInfo, PERFORMANCE_LEVEL PerformanceLevel);

// This function expands the Local Pool in the specified allocation space of process
BOOL    RTAPI RtExpandMSpace(mspace MSpace, ULONG Size);

// This function shrinks the Local Pool in the specified allocation space of process
ULONG   RTAPI RtShrinkMSpace(mspace MSpace, ULONG Size);

// This function allocates memory from the Local Memory pool in the external MSpace of the current process.
// Note that newly allocated Local Memory is zeroed only if configured to do so using the RTX64 Control Panel,
// native API RtfwSetLocalMemoryConfiguration, or managed API
// IntervalZero.RTX64.Config.Subsystem.ZeroMemoryAtAllocation.  By default, RTX64 does NOT zero newly
...
typedef struct _RTPROCESS_INFORMATION {
	...
	unsigned __int64	ProcessorAffinityMask;		// A bit vector, each bit represents an allowed processor
...
typedef enum _RT_PROC_THREAD_ATTRIBUTE_NUM {
	RtProcThreadAttributeIdealProcessor = 0x1009, // This is a magic number, so don't change it.
	RtProcThreadAttributeProcessAffinity,
    RtProcThreadAttributeMSpaceMinimumInitialSize,
    RtProcThreadAttributMSpaceExpandSize,
...
#define RT_PROC_THREAD_ATTRIBUTE_MSPACE_MINIMUM_INITIAL_SIZE \
	ProcThreadAttributeValue(RtProcThreadAttributeMSpaceMinimumInitialSize, FALSE, FALSE, FALSE)
#define RT_PROC_THREAD_ATTRIBUTE_MSPACE_EXPAND_SIZE \
	ProcThreadAttributeValue(RtProcThreadAttributMSpaceExpandSize, FALSE, FALSE, FALSE)
...
// This function retrieves the ideal processor for a given process handle.
BOOL	RTAPI RtGetProcessIdealProcessor(HANDLE hProcess, PULONG pIdealProcessor);
...
BOOL	RTAPI RtGetProcessAffinityMask(HANDLE hProcess, PUINT64 ProcessAffinityMask, PUINT64 SystemAffinityMask);
// This function sets a processor affinity mask for the threads of the specified RTSS process.
BOOL	RTAPI RtSetProcessAffinityMask(HANDLE hProcess, UINT64 ProcessAffinityMask);
// This function gets the number represention for the lowest and highest rtss processors
BOOL	RTAPI RtGetProcessorInfo(PDWORD lpLowestSystemProcessor, PDWORD lpHighestSystemProcessor);
...
// This function retrieves process info for the passed process ID
BOOL RTAPI RtGetProcessInfoByProcessId(DWORD dwProcessId, PRT_PROCESS_INFO pProcessInfo);
...
//
// This structure defines shared memory information retrieved by RtQuerySharedMemory 
//
typedef struct _SHARED_MEMORY_INFO
{
	DWORD flProtect; //Protection attribute for shared memory
	DWORD dwSize;    //Total size of the shared memory
}SHARED_MEMORY_INFO, *PSHARED_MEMORY_INFO;

// This function obtains the shared memory information.
BOOL RTAPI RtQuerySharedMemory(HANDLE hHandle,	PSHARED_MEMORY_INFO pSharedMemoryInfo);
...
#define	MAX_WFMO	64	// Maximum number of objects per WFMO
...
typedef enum _MF_EVENT_KIND
{
	...
	MF_EVENT_KIND_THREAD_OPEN,                                   // 103
	MF_EVENT_KIND_CLOS_SET,                                      // 104
	MF_EVENT_KIND_WATCHDOG_RESET,								 // 105
    MF_EVENT_KIND_HEAP_REALLOC,                                  // 106
...
typedef enum _RTX64_MONITOR_CONTROL_OP
{
    ...
    // monitoring is already started or paused, this operation fails with ERROR_INVALID_FUNCTION.
    //
    ...
    // started or paused, otherwise function RtMonitorControl will fail and set the last error value to
    // ERROR_NOT_READY.
    //
    // Parameters "data" and "size" are ignored for this operation.
	...
	// This operation deterministically pauses RTX64 monitoring.  This will succeed only when monitoring is
	// enabled and started.  If monitoring is not enabled, this operation fails with ERROR_NOT_READY.  If
	// monitoring is either stopped or paused, this operation fails with ERROR_INVALID_FUNCTION.
	//
	// Parameters "data" and "size" are ignored for this operation.
	MONITOR_CONTROL_PAUSE,

	// This operation deterministically resumes RTX64 monitoring from paused state.  This will succeed only
	// when monitoring is paused.  If monitoring is not enabled, this operation fails with ERROR_NOT_READY.
	// If monitoring is in any other state, this operation fails with ERROR_INVALID_FUNCTION.
	//
	// Parameters "data" and "size" are ignored for this operation.
	MONITOR_CONTROL_RESUME,

    // This operation transiently enables generation of monitoring events by one or more RTX64 product
    // components.  Changes made by this operation are transient, and thus last only until the RTX64 subsystem
    // stops.  To persistently change which components have event generation enabled, use managed API
    // Monitor.Subsystem.ToggleComponents.
    //
    // Parameter "data" points to a 64-bit unsigned integer that is the bitwise OR of enumerators from
    // enumeration RT_MONITOR_COMPONENT, where each set bit represents a product component that will have its
    // event generation enabled.  Parameter "size" must be set to 8 (e.g.,
    ...
    // This operation retrieves the transient configuration of RTX64 product components that currently have
    // monitoring event generation enabled.  Parameter "data" points to a 64-bit unsigned integer that is set
    // to the bitwise OR of enumerators from enumeration RT_MONITOR_COMPONENT, where each set bit represents a
    // product component that has event generation enabled.  Parameter "size" must bet set to 8.
...
// This function starts, stops, pauses, or resumes the RTX64 Monitoring Framework.  Parameter
// monitorStateChange must be one of:
//
//    MONITOR_CONTROL_START
//    MONITOR_CONTROL_PAUSE
//    MONITOR_CONTROL_STOP
...
//
// This function returns TRUE if successful.  Otherwise, it returns FALSE and sets the last error value.
BOOL RTAPI RtMonitorChangeState(RTX64_MONITOR_CONTROL_OP monitorStateChange);

// Used for the function RtMonitorSetEvents as a sentinal value.  This lets the function know it needs
// apply this change to ALL events
#define MF_ALL_STANDARD_EVENTS ((MF_EVENT_KIND *) 0xffffffffffffffff)

// Used for the function RtMonitorSetCustomTriggers as a sentinal value.  This lets the function know it
// needs apply this change to ALL custom events
#define MF_ALL_CUSTOM_EVENTS ((MF_EVENT_KIND *) 0xffffffffffffffff)

// This struct is used to associate trigger values to certain MF_EVENT_KINDS.  
...
// Labeling this to a more readable value type.
typedef int CUSTOM_EVENT_KIND;

// This struct is used to associate trigger values to certain custom Events.  
typedef struct _RT_MONITOR_CUSTOM_TRIGGER_CONTROL
{
	size_t size;
	MF_TRIGGERS triggers;
	CUSTOM_EVENT_KIND customEvent;
} RT_MONITOR_CUSTOM_TRIGGER_CONTROL, *PRT_MONITOR_CUSTOM_TRIGGER_CONTROL;

// This function will enable or disable monitoring events.  
BOOL RTAPI RtMonitorSetEvents(const MF_EVENT_KIND * eventKinds, size_t countEventKinds, BOOL enable);

// This function will enable or disable monitoring triggers.
BOOL RTAPI RtMonitorSetTriggers(const RT_MONITOR_TRIGGER_CONTROL * eventTriggers, size_t countEventTriggers, BOOL setTriggers);

// This function will enable or disable triggers for custom events.
BOOL RTAPI RtMonitorSetCustomTriggers(const RT_MONITOR_CUSTOM_TRIGGER_CONTROL * CustomEventTriggers,
                                         size_t countCustomEventTriggers, BOOL setTriggers);

// This enumeration represents RTX64 product components that can have their monitoring event generation
// independently enabled/disabled, either persistently or transiently.  If one of these components has event
// generation disabled, no monitoring events will be generated by that product component, regardless of
// which individual monitoring events are enabled.
typedef enum _RT_MONITOR_COMPONENT
{
    RT_MONITOR_COMPONENT_NONE       = 0,  // Represents no product components. 
    RT_MONITOR_COMPONENT_TCPIPSTACK = 1,  // Represents the RTX64 real-time TCP/IP stack.
    RT_MONITOR_COMPONENT_NAL        = 2,  // Represents the RTX64 real-time Network Abstraction Layer (NAL).

    RT_MONITOR_COMPONENT_ALL        = 3,  // Rrepresents the union of all of the above product components.
} RT_MONITOR_COMPONENT, * PRT_MONITOR_COMPONENT;

// This function transiently enables/disables the generation of monitor events by one or more RTX64 product components.
// Changes made by this function are transient, and thus last only until the RTX64 subsystem stops.  To
// persistently change which components have event generation enabled, use managed API
// Monitor.Subsystem.ToggleComponents.
//
// Parameter "components" is the bitwise OR of enumerators from enumeration RT_MONITOR_COMPONENT, where each
// set bit represents a product component that will have its event generation enabled or
// disabled depending on the enable parameter.
// 
// Parameter "enable" determines if the bits that are set to 1 will either explicitly enable the component or disable the component
//
// If successful, this function returns TRUE, otherwise it returns FALSE and sets the last error value.  Use
// function RtMonitorGetEnabledComponents to discover which product components are currently enabled.
BOOL RTAPI RtMonitorEnableComponents(unsigned __int64 components, BOOL enable);

// This function retrieves the RTX64 product components that currently have monitoring event generation
// enabled.  Parameter "pComponents" points to a 64-bit unsigned integer that is set to the bitwise OR of
// enumerators from enumeration RT_MONITOR_COMPONENT, where each set bit represents a product component that
// has event generation enabled.
//
// If successful, this function returns TRUE, otherwise it returns FALSE and sets the last error value.
BOOL RTAPI RtMonitorGetEnabledComponents(unsigned __int64 * pComponents);
...
#ifdef UNICODE
# define _tRtTtoi RtWtoi
#else
# define _tRtTtoi RtAtoi
#endif
...
#ifdef UNICODE
# define _tRtPrintf RtWprintf
#else
# define _tRtPrintf RtPrintf
#endif

// Flags for RtLogTextA logging functionality
typedef enum _RT_LOG_TEXT_FLAGS
{
	RT_LOG_TEXT_FLAG_NONE = 0,	// Do nothing.
	RT_LOG_TEXT_FLAG_WRITE_CONSOLE = 1,	// Print to console.
	RT_LOG_TEXT_FLAG_LOG_INFO = 2,	// Log to Rtx64Log as info
	RT_LOG_TEXT_FLAG_LOG_WARNING = 4,	// Log to Rtx64Log as warning
	RT_LOG_TEXT_FLAG_LOG_ERROR = 8,	// Log to Rtx64Log as error
	RT_LOG_TEXT_FLAG_LOG_POPUP = 16,	// Display a popup 
} RT_LOG_TEXT_FLAGS;

#ifdef UNDER_RTSS

int _cdecl RtLogTextW(DWORD loggingFlags, LPCWSTR format, ...);
int _cdecl RtLogTextA(DWORD loggingFlags, LPCSTR format, ...);

#ifdef UNICODE
#define RtLogText RtLogTextW
#else
#define RtLogText RtLogTextA
#endif
...
// This function retrieves timing information for the specified RTSS process.

BOOL RTAPI RtGetProcessTimes(
	HANDLE hProcess,
...
//
// This function retrieves array of system timing information. 
// On a multiprocessor system, values in each of the FILETIME structures will contain system timing 
// information about the RTSS core.
//
BOOL RTAPI RtGetSystemTimes(
	LPFILETIME lpCoreIdleTimes,
	LPFILETIME lpCoreExecuteTimes,
	ULONG *lpSize
);
