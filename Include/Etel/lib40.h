/*
 *
 * Copyright (c) 1997-2017 ETEL SA. All Rights Reserved.
 *
...
/* VXWORKS_6_9 NATIVE thread macros	    */
	...
	#define THREAD_INIT_PRIO(thr, fct, arg, prio)        do {\
	   	   	   	   	   	   	   	   	   	   	   	   	   		thr = taskSpawn(NULL, prio, VX_FP_TASK | VX_NO_STACK_PROTECT, 128000, (FUNCPTR)fct, (int)arg, 0, 0, 0, 0, 0, 0, 0, 0, 0);\
														} while(0)
	#define THREAD_INIT_AND_NAME_PRIO(thr,name,fct,arg,prio) do {\
															thr = taskSpawn(name, prio, VX_FP_TASK | VX_NO_STACK_PROTECT, 128000, (FUNCPTR)fct, (int)arg, 0, 0, 0, 0, 0, 0, 0, 0, 0);\
														} while(0)
	...
	#define THREAD_INIT(thr,fct,arg)                    do { \
															thr = taskSpawn(NULL, EDI_THREAD_PRIORITY_NORMAL, VX_FP_TASK | VX_NO_STACK_PROTECT, 128000, (FUNCPTR)fct, (int)arg, 0, 0, 0, 0, 0, 0, 0, 0, 0); \
														}while (0)
	#define THREAD_INIT_AND_NAME(thr,name,fct,arg)      do {\
															thr = taskSpawn(name, EDI_THREAD_PRIORITY_NORMAL, VX_FP_TASK | VX_NO_STACK_PROTECT, 128000, (FUNCPTR)fct, (int)arg, 0, 0, 0, 0, 0, 0, 0, 0, 0); \
														}while (0)
...
/* VXWORKS NATIVE Critical section  	*/
	...
	#define CRITICAL                struct {DEFINE_CRITICAL}
	#define CRITICAL_INIT(ob)       do { \
										(ob).sem = semMCreate(SEM_Q_PRIORITY | SEM_INVERSION_SAFE | SEM_USER); \
									} while(0)
	#define CRITICAL_DESTROY(ob)    do { \
										semDelete((ob).sem); \
									} while(0)
...
/* VXWORKS NATIVE mutexes macros		*/
	...
	#define MUTEX_INVALID           NULL
	#define MUTEX_INIT(mut,init)    do { \
										mut = semMCreate(SEM_Q_PRIORITY | SEM_DELETE_SAFE | SEM_INVERSION_SAFE | SEM_USER | SEM_INTERRUPTIBLE); \
										if (init) \
