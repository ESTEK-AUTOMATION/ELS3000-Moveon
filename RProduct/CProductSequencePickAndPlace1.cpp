...
int CProductSequence::PickAndPlace1Sequence()
{
	...
	bool bS1VisionSendRowColumnDone = false;
	...
	while (smProductEvent->ExitRTX.Set == false)
	{
		if (nSequenceNo != nSequenceNo_Prev)
		{
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: nCase %u\n", nHeadNo, nSequenceNo);
		}
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceSequence:
			if (smProductEvent->RSEQ_RPNP1_SEQUENCE_START.Set == true)
			{
				lnPnPSequenceClockStartCycle.QuadPart = 0;
				smProductEvent->RSEQ_RPNP1_SEQUENCE_START.Set = false;
				smProductEvent->RTHD_GMAIN_UPDATE_IN_PROGRESS_MAPPING.Set = false;
				smProductEvent->RTHD_GMAIN_UPDATE_INPUT_MAPPING.Set = false;
				smProductEvent->RMAIN_RTHD_UPDATE_MAP_AFTER_S1_MISSING_UNIT.Set = false;
				smProductEvent->RMAIN_RTHD_UPDATE_MAP_AFTER_S3_MISSING_UNIT.Set = false;
				smProductEvent->RMAIN_RTHD_UPDATE_MAP_AFTER_OUTPUT_MISSING_UNIT.Set = false;
				smProductEvent->RMAIN_RTHD_UPDATE_MAP_AFTER_OUTPUT_POST_MISSING_UNIT.Set = false;
				smProductEvent->RMAIN_RTHD_UPDATE_MAP_AFTER_REJECT_POST_MISSING_UNIT.Set = false;
				smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set = false;
				//smProductProduction->bPNP1AllowS2S3Snap = true;
				smProductProduction->CurrentS2S3FacetTotalSnap = 0;
				for (int i = smProductProduction->CurrentS2VisionLoopNo; i < 10; i++)
				{
					if (smProductSetting->S2FacetVision[i].ThetaOffset_mDegree > 0)
					{					
						smProductProduction->CurrentS2S3FacetTotalSnap++;
					}
					else
					{
						break;
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToStandbyPosition:
			if (smProductEvent->RSEQ_RPNP1_STANDBY_START.Set == true)
			{
		...
		case nCase.StartMovePickAndPlaceXYAxisToStandbyPosition:
			...
			{
				if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.BottomStation)
				{
				...
				else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation)
				{
					smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisParkingPosition;
					smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisStandbyPosition;
				...
				else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.S3Station)
				{
					...
					smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisAtS2AndS3VisionPosition;
					smProductEvent->PickAndPlace1XAxisMotorMoveToS3PositionDone.Set = false;
					smProductEvent->StartPickAndPlace1XAxisMotorMoveToS3Position.Set = true;
					smProductEvent->PickAndPlace1YAxisMotorMoveToS3PositionDone.Set = false;
					smProductEvent->StartPickAndPlace1YAxisMotorMoveToS3Position.Set = true;
		...
		case nCase.IsMovePickAndPlaceXYAxisToStandbyPositionDone:
			...
			if ((smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0]
				&& ((smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.BottomStation && smProductEvent->PickAndPlace1YAxisMotorMoveToS1PositionDone.Set == true && smProductEvent->PickAndPlace1XAxisMotorMoveToS1PositionDone.Set == true)
					|| (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.S3Station && smProductEvent->PickAndPlace1YAxisMotorMoveToS3PositionDone.Set == true && smProductEvent->PickAndPlace1XAxisMotorMoveToS3PositionDone.Set == true))
				...
				|| smProductCustomize->EnablePickAndPlace1XAxisMotor == false || smProductCustomize->EnablePickAndPlace1YAxisMotor == false || smProductSetting->EnablePH[0] == false)
			{
				if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.BottomStation)
				{
					...
					smProductEvent->RPNP1_RSEQ_MOVE_TO_BOTTOM_STATION_DONE.Set = true;
					smProductEvent->RPNP1_RSEQ_PROCESS_AT_BOTTOM_STATION_DONE.Set = true;
				...
				else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.S3Station)
				{
					...
					smProductProduction->PickAndPlace1StationToMove = PickAndPlaceCurrentStation.UnknownStation;
					smProductEvent->RPNP1_RSEQ_MOVE_TO_S3_STATION_DONE.Set = true;
			...
			{
				if (smProductCustomize->EnablePickAndPlace1XAxisMotor)
				{
		...
		case nCase.IsStopMovePickAndPlaceXYAxisToStandbyPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductEvent->PickAndPlace1XAxisMotorStopDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorStopDone.Set == true)
		...
		case nCase.IsMovePickAndPlaceYAxisToParkingPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductEvent->PickAndPlace1YAxisMotorMoveToStandbyPositionDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
			{
		...
		case nCase.IsStopMovePickAndPlaceYAxisToParkingPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductEvent->PickAndPlace1YAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
			{
		...
		case nCase.IsMovePickAndPlaceXAxisToParkingPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductEvent->PickAndPlace1XAxisMotorMoveToParkingPositionDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1XAxisMotor == false)
			{
		...
		case nCase.IsStopMovePickAndPlaceXAxisToParkingPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductEvent->PickAndPlace1XAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1XAxisMotor == false)
			{
		...
		case nCase.SetEventPickAndPlaceToStandbyPositionDone:
			smProductEvent->RPNP1_RSEQ_STANDBY_DONE.Set = true;
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToPickOrPlacePosition:
			if (smProductEvent->RSEQ_RPNP1_MOVE_TO_INPUT_STATION_START.Set == true
				|| smProductEvent->RSEQ_RPNP1_MOVE_TO_OUTPUT_STATION_START.Set == true)
			{
				...
				smProductEvent->RSEQ_RPNP1_MOVE_TO_OUTPUT_STATION_START.Set = false;
			...
			else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.S3Station && smProductEvent->RPNP2_RSEQ_POST_PRODUCTION_DONE.Set == true)
			{
				nSequenceNo = nCase.StartMovePickAndPlaceXYAxisToStandbyPosition;
			}
			//else if (smProductEvent->RSEQ_RPNP1_POST_PRODUCTION_START.Set == true)
			//{
		...
		case nCase.StartMovePickAndPlaceXYAxisToPickOrPlacePosition:
			...
			{
				if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.InputStation)
				{
					smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisInputPosition + (signed long)smProductSetting->PickingCenterXOffsetInput;
					smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisInputPosition + (signed long)smProductSetting->PickingCenterYOffsetInput;
					...
					smProductEvent->StartPickAndPlace1YAxisMotorMoveToInputPosition.Set = true;
				}
				else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.OutputStation)
				{
					smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisOutputPosition;
					smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisOutputPosition;
					smProductEvent->PickAndPlace1XAxisMotorMoveToOutputPositionDone.Set = false;
					smProductEvent->StartPickAndPlace1XAxisMotorMoveToOutputPosition.Set = true;
					smProductEvent->PickAndPlace1YAxisMotorMoveToOutputPositionDone.Set = false;
					smProductEvent->StartPickAndPlace1YAxisMotorMoveToOutputPosition.Set = true;
				}
			}
		...
		case nCase.CheckIsPickAndPlaceYAxisReadyToMovePositionCurve:
			if (IsPickAndPlace1YAxisSaveToMoveCurve() == true)
			{
				if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.InputStation)
				{
					if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0])
					{
						...
						smProductEvent->StartPickAndPlace1YAxisMotorMoveToInputPositionCurve.Set = true;
					}
					smProductProduction->bPNP1AllowS2S3Snap = true;
					nSequenceNo = nCase.IsMovePickAndPlaceXYAxisToPickOrPlacePositionDone;
				}
		...
		case nCase.IsMovePickAndPlaceXYAxisToPickOrPlacePositionDone:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0]
				&& (((smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.InputStation && smProductEvent->PickAndPlace1XAxisMotorMoveToInputPositionDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveToInputPositionDone.Set == true)
					&& ((smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveToInputPositionCurveDone.Set == true) || smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == false))
					|| ((smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.OutputStation && smProductEvent->PickAndPlace1XAxisMotorMoveToOutputPositionDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveToOutputPositionDone.Set == true)
						&& ((smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveToOutputPositionCurveDone.Set == true) || smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == false))
					)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.InputStation)
				{
					...
					smProductEvent->RPNP1_RSEQ_MOVE_TO_INPUT_STATION_DONE.Set = true;
				}
				else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.OutputStation)
				{
					...
					smProductEvent->RPNP1_RSEQ_MOVE_TO_OUTPUT_STATION_DONE.Set = true;
		...
		case nCase.IsStopMovePickAndPlaceXYAxisToPickOrPlacePositionDone:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0]
				&& smProductEvent->PickAndPlace1XAxisMotorStopDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorStopDone.Set == true
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToPickPosition:
			...
			{
				smProductEvent->RSEQ_RPNP1_MOVE_TO_INPUT_STATION_START.Set = false;
				smProductProduction->nCurrentPickupHeadAtInput = 0;
		...
		case nCase.StartMovePickAndPlaceXYAxisToPickPosition:
		{
			...
			{
				nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 1000000, 500, 10000000, 10000000); ////1200000--> 600000 27Nov2024 //back to 1200000
				//nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 800000, 500, 8500000, 8500000);
			...
			if (smProductProduction->PickAndPlace1XAxisMovePosition > smProductTeachPoint->PickAndPlace1XAxisInputPosition + (signed long)2000 ||
				smProductProduction->PickAndPlace1XAxisMovePosition < smProductTeachPoint->PickAndPlace1XAxisInputPosition - (signed long)2000)
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Position is out of Pick And Place 1 X Input Station Limit\n");
			...
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Position is out of Pick And Place 1 Y Input Station Limit\n");
			...
			if (smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == true)
			{
				...
				if (m_cProductIOControl->IsPickAndPlace1XAxisMotorInPosition() == false)
				{
					m_cProductShareVariables->SetAlarm(55006);
					m_cProductShareVariables->UpdateMessageToGUIAndLog("Pick And Place 1 X Axis Motor is busy.");
					break;
				}
				...
				if (m_cProductIOControl->IsPickAndPlace1YAxisMotorInPosition() == false)
				{
					m_cProductShareVariables->SetAlarm(40006);
					m_cProductShareVariables->UpdateMessageToGUIAndLog("Pick And Place 1 Y Axis Motor is busy.");
					break;
				}
			}
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0])
			...
			smProductProduction->PickAndPlace1ThetaAxisMovePosition = smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition;// +smProductSetting->UnitPlacementRotationOffsetInput;
			...
			m_cLogger->WriteLog("PickAndPlace1XAxisInputPosition %lf.\n", (double)(smProductTeachPoint->PickAndPlace1XAxisInputPosition));
			m_cLogger->WriteLog("PickAndPlace1YAxisMovePosition %lf.\n", (double)(smProductProduction->PickAndPlace1YAxisMovePosition));
			m_cLogger->WriteLog("PickAndPlace1YAxisInputPosition %lf.\n", (double)(smProductTeachPoint->PickAndPlace1YAxisInputPosition));
			m_cLogger->WriteLog("PickAndPlace1ThetaAxisMovePosition %lf.\n", (double)(smProductProduction->PickAndPlace1ThetaAxisMovePosition));
			m_cLogger->WriteLog("PickAndPlace1ThetaAxisStandbyPosition %lf.\n", (double)(smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition));
			m_cLogger->WriteLog("InputThetaOffset %lf.\n", (double)smProductProduction->InputThetaOffset);

			smProductEvent->RPNP1_ROUT_THREAD_AWAY_OUTPUT.Set = true;
			if (smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == true)
			{
		...
		case nCase.MoveYToStanbyBeforeMoveXToInput:
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0])
			{
				smProductEvent->PickAndPlace1YAxisMotorMoveDone.Set = false;
				smProductEvent->StartPickAndPlace1YAxisMotorMove.Set = true;
			}
		...
		case nCase.IsMoveYToStanbyBeforeMoveXToInputDone:
			...
			if ((smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0]
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductSetting->EnablePH[0])
				{
					smProductEvent->PickAndPlace1XAxisMotorMoveDone.Set = false;
					smProductEvent->StartPickAndPlace1XAxisMotorMove.Set = true;					
				}
		...
		case nCase.CheckIsPickAndPlaceXAxisReadyMoveToPickCurvePositionForS2S3VisionInspection:
			...
			{
				if (m_cProductMotorControl->ReadPickAndPlace1XAxisMotorEncoder() > (smProductTeachPoint->PickAndPlace1XAxisOutputPosition + (signed long)50000))
		...
		case nCase.IsMoveXAndThetaToInputDone:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductSetting->EnablePH[0]
				&& ((smProductEvent->PickAndPlace1XAxisMotorMoveDone.Set == true && smProductEvent->PickAndPlace1ThetaAxisMotorMoveDone.Set == true))
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{						
		...
		case nCase.MoveYToInputAfterMoveXToInput:
			...
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0])
			{
				...
				smProductEvent->StartPickAndPlace1YAxisMotorMoveCurve.Set = true;
			}			
		...
		case nCase.IsMoveYToInputAfterMoveXToInputDone:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0]
				&& ((smProductEvent->PickAndPlace1XAxisMotorMoveDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveDone.Set == true)
					&& ((smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveCurveDone.Set == true) || smProductEvent->PickAndPlace1YAxisMotorMoveCurve.Set == false))
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				m_cLogger->WriteLog("PickAndPlace1ZAxisCurrentEncoderPosition %lf.\n", (double)(m_cProductMotorControl->ReadPickAndPlace1ZAxisMotorEncoder()));
				m_cLogger->WriteLog("PickAndPlace1ThetaAxisCurrentEncoderPosition %lf.\n", (double)(m_cProductMotorControl->ReadPickAndPlace1ThetaAxisMotorEncoder()));
		...
		case nCase.IsMovePickAndPlaceXYAxisToPickPositionDone:
			...
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0]
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				if ((double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) < smProductProduction->PickAndPlace1XAxisMovePosition - 200
					|| (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) > smProductProduction->PickAndPlace1XAxisMovePosition + 200)
				{
					m_cLogger->WriteLog("PickAndPlace1XAxisCurrentEncoderPosition when Input not going position %lf.\n", (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)));
					m_cLogger->WriteLog("PickAndPlace1XAxisMovePosition when Input not going position %lf.\n", (double)(smProductProduction->PickAndPlace1XAxisMovePosition));
					nSequenceNo = nCase.StartMovePickAndPlaceXYAxisToPickPosition;
					RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
					break;
				}
		...
		case nCase.StopMovePickAndPlaceXYAxisToPickPosition:
			...
			smProductEvent->PickAndPlace1ThetaAxisMotorStopDone.Set = false;
			smProductEvent->StartPickAndPlace1ThetaAxisMotorStop.Set = true;
			m_cLogger->WriteLog("PickAndPlaceSeq%d: Stop Move PickAndPlace XY Theta Axis To Pick Position.\n", nHeadNo);
		...
		case nCase.StartMovePickAndPlaceZToPurgingPosition:
			...
			//	break;
			//}
			smProductProduction->PickAndPlace1ZAxisMovePosition = smProductTeachPoint->PickAndPlace1ZAxisUpPosition - (signed long)3000;
			if (smProductCustomize->EnablePickAndPlace1Module && smProductSetting->EnablePH[0])
			{
				smProductEvent->PickAndPlace1ZAxisMotorMoveDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisMotorMove.Set = true;
			}
		...
		case nCase.IsMovePickAndPlaceZToPurgingPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] == true && smProductEvent->PickAndPlace1ZAxisMotorMoveDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				//	break;
				//}
				smProductEvent->PickAndPlace1ZAxisReleaseAndOffValveDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisReleaseAndOffValve.Set = true;
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			}
			else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->MOTION_TIMEOUT)
			{
		...
		case nCase.IsStopMovePickAndPlaceZToPurgingPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1Module && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.IsPickAndPlaceReleaseAndOffValveDone:
			...
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductEvent->PickAndPlace1ZAxisReleaseAndOffValveDone.Set == true)
			{
		...
		case nCase.IsPostProductionDone:
			...
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductEvent->RSEQ_RPNP1_REMOVE_UNIT_SKIP.Set == true)
			{
				...
				else if (smProductProduction->PickAndPlace1CurrentStation == PickAndPlaceCurrentStation.DisableStation)
					nSequenceNo = nCase.WaitingToReceiveEventStartPickAndPlaceToPickPosition;
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			}
			else if (smProductEvent->RSEQ_RPNP1_REMOVE_UNIT_START.Set == true)//place fail
			{
		...
		case nCase.StartMovePickAndPlaceYZToStandbyPositionAfterPurging:
			...
			if (smProductCustomize->EnablePickAndPlace1Module && smProductSetting->EnablePH[0])
			{
				smProductProduction->PickAndPlace1ZAxisMovePosition = smProductTeachPoint->PickAndPlace1ZAxisUpPosition;
				smProductEvent->PickAndPlace1ZAxisMotorMoveDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisMotorMove.Set = true;
			}
		...
		case nCase.IsMovePickAndPlaceYZToStandbyPositionAfterPurgingDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (((smProductCustomize->EnablePickAndPlace1YAxisMotor == true && smProductEvent->PickAndPlace1YAxisMotorMoveToStandbyPositionDone.Set == true) || smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
				&&((smProductCustomize->EnablePickAndPlace1Module && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorMoveDone.Set == true)
					|| smProductCustomize->EnablePickAndPlace1Module==false|| smProductSetting->EnablePH[0]==false)
				)
			{
		...
		case nCase.IsStopMovePickAndPlaceYZToStandbyPositionAfterPurgingDone:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (
				((smProductCustomize->EnablePickAndPlace1YAxisMotor == true && smProductEvent->PickAndPlace1YAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
				&&((smProductCustomize->EnablePickAndPlace1Module && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorStopDone.Set == true)
					|| smProductCustomize->EnablePickAndPlace1Module == false|| smProductSetting->EnablePH[0]==false)
				)
			{
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToOutputPositionToPick:
			if (smProductEvent->RSEQ_RPNP1_MOVE_TO_OUTPUT_STATION_START.Set == true)
			{
				smProductEvent->RSEQ_RPNP1_MOVE_TO_OUTPUT_STATION_START.Set = false;
		...
		case nCase.CheckPickAndPlaceModuleSafeToMoveToOutputPositionToPick:
			if (/*smProductEvent->RPNP2_RSEQ_MOVE_STANDBY_DONE.Set ==*/ true)
			{
		...
		case nCase.IsMovePickAndPlaceYToStandbyPositionBeforeOutputPositionToPickDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1YAxisMotor == true && smProductEvent->PickAndPlace1YAxisMotorMoveToStandbyPositionDone.Set == true ) 
				|| smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
			{
		...
		case nCase.IsStopMovePickAndPlaceYToStandbyPositionBeforeOutputPositionToPickDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1YAxisMotor == true && smProductEvent->PickAndPlace1YAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
			{
		...
		case nCase.StartMovePickAndPlaceXYZThetaAxisToOutputPositionToPick:
		{
			...
			//	, (double)smProductProduction->OutputTableResult[0].OutputXOffset_um_Post - (double)(smProductSetting->PickUpHeadRotationXOffset[0])
			//	, (double)smProductProduction->OutputTableResult[0].OutputYOffset_um_Post - (double)(smProductSetting->PickUpHeadRotationYOffset[0])
			//	, &dCalculatedOutputVisionXOffset, &dCalculatedOutputVisionYOffset);

			//GetNewXYOffsetFromThetaCorretion((double)smProductSetting->PickUpHeadOutputCompensationThetaOffset[0]
			//	, (double)smProductSetting->PickUpHeadOutputCompensationXOffset[0]
			//	, (double)smProductSetting->PickUpHeadOutputCompensationYOffset[0]
			//	, &dCalculatedOutputXOffset, &dCalculatedOutputYOffset);

			smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisOutputPosition
				+ signed long(dCalculatedOutputVisionXOffset + (smProductSetting->PickUpHeadRotationXOffset[0]))
				- dCalculatedOutputXOffset;

			smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisOutputPosition
				+ signed long(dCalculatedOutputVisionYOffset + (smProductSetting->PickUpHeadRotationYOffset[0]))
				- dCalculatedOutputYOffset;
			if(m_bHighSpeed)
				nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 1200000, 500, 10000000, 10000000);
			else
			{
				nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 1000000, 500, 10000000, 10000000); //700000--> 350000 27Nov2024 // back to 700000
				//nError = m_cProductMotorControl->AgitoMotorSpeed(0,0, 700000, 500, 8500000, 8500000);
				//nError = m_cProductMotorControl->AgitoMotorSpeed(0,0,50000, 500,250000,250000);
			}
			...
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Position is out of Pick And Place 1 X Output Station Limit\n");
				m_cProductShareVariables->SetAlarm(55010);
				break;
			}
			...
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Position is out of Pick And Place 1 Y Output Station Limit\n");
				m_cProductShareVariables->SetAlarm(40010);
				break;
			}
			...
			smProductProduction->PickAndPlace1ThetaAxisMovePosition = (smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition - smProductSetting->OutputVisionUnitThetaOffset - smProductProduction->OutputTableResult[0].OutputThetaOffset_mDegree_Post - smProductSetting->PickUpHeadOutputCompensationThetaOffset[0]);
			smProductProduction->PickAndPlace1ZAxisMovePosition = smProductTeachPoint->PickAndPlace1ZAxisUpPosition2 + (signed long)smProductSetting->UnitThickness_um;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0])
			{
				smProductEvent->PickAndPlace1ZAxisMotorMoveUpPositionAndRotateDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisMotorMoveUpPositionAndRotate.Set = true;
		...
		case nCase.CheckIsPickAndPlaceYAxisReadyToMoveCurveOutputPositionToPick:
			if (IsPickAndPlace1YAxisSaveToMoveCurveOutput() == true)
			{
				if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductSetting->EnablePH[0])
				{
					smProductEvent->PickAndPlace1YAxisMotorMoveCurveDone.Set = false;
		...
		case nCase.IsMovePickAndPlaceXYZThetaAxisToOutputPositionDoneToPick:
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				smProductProduction->PickAndPlace1CurrentStation = PickAndPlaceCurrentStation.OutputStation;
				smProductEvent->RPNP1_RSEQ_MOVE_TO_OUTPUT_STATION_DONE.Set = true;
				smProductEvent->RPNP1_RSEQ_Y_MOVE_STANDBY.Set = true;
				smProductEvent->RINP_RPNP_REQUIRE_PURGING.Set = false;
		...
		case nCase.IsStopMovePickAndPlaceXYZThetaAxisToOutputPositionDoneToPick:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0]
				&& (smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductEvent->PickAndPlace1XAxisMotorStopDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorStopDone.Set == true
					|| smProductCustomize->EnablePickAndPlace1XAxisMotor == false || smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
				&& (smProductEvent->PickAndPlace1ThetaAxisMotorStopDone.Set == true && smProductEvent->PickAndPlace1ZAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.IsTrayTableReadyToPickAfterFail:
			...
			else if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 0 && 
			...
			{
				smProductProduction->nCurrentPickAndPlace1PickingRetry = 0;
		...
		case nCase.StartMovePickAndPlaceZAxisToDownPlacePositionToPick:
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0])
			{
				smProductEvent->PickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPositionForPickingDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPositionForPicking.Set = true;
			}
		...
		case nCase.IsMovePickAndPlaceZAxisToDownPlacePositionDoneToPick:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPositionForPickingDone.Set == true
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
			...
			{
				m_cProductShareVariables->SetAlarm(11027);
		...
		case nCase.IsStopMovePickAndPlaceZAxisToDownPlacePositionDoneToPick:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorStopDone.Set == true
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.StartPickAndPlaceZAxisOffVacuumForOutputPickFail:
			...
			{
				smProductEvent->PickAndPlace1ZAxisOffVacuumDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisOffVacuum.Set = true;
			}
		...
		case nCase.IsPickAndPlaceZAxisOffVacuumForOutputPickFailDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductEvent->PickAndPlace1ZAxisOffVacuumDone.Set == true && smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.StartPickAndPlaceZAxisValveOnForOutputPickFail:
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] == true)
			{
				smProductEvent->PickAndPlace1ZAxisOnValveDone.Set = false;
				smProductEvent->StartPickAndPlace1ZAxisOnValve.Set = true;
			}
		...
		case nCase.IsPickAndPlaceZAxisValveOnForOutputPickFailDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductEvent->PickAndPlace1ZAxisOnValveDone.Set == true && smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.IsDelayForPurgingReachedBeforeOffValveForOutputPickFail:
			...
			if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount >= smProductSetting->DelayForPickupHeadPurgeAtDownPositionWhenPickFail)
			{
				...
				if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] == true)
				{
					smProductEvent->PickAndPlace1ZAxisOffValveDone.Set = false;
					smProductEvent->StartPickAndPlace1ZAxisOffValve.Set = true;
				}
		...
		case nCase.IsPickAndPlaceZAxisValveOffForOutputPickFailDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductEvent->PickAndPlace1ZAxisOffValveDone.Set == true && smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.IsMovePickAndPlaceZAxisToUpPositionDoneAfterPick:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorMoveUpPositionDone.Set == true
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Move PickAndPlace Z Axis To Up Position Done After Pick %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
				//if (smProductEvent->PickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPositionForPickingDone.Set == false)
				//{
				//	nSequenceNo = nCase.StartMovePickAndPlaceZAxisToDownPlacePositionToPick;
				//}
				//else
		...
		case nCase.IsPickAndPlacePickZAxisAtUpPositionDoneAfterPick:
		{
			...
			if (dblReturnValue <= smProductSetting->PickUpHead1Pressure)
			{
				...
				smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 1;
				...
				if (smProductEvent->PickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPositionForPickingDone.Set == true)
				{
				...
				{
					m_cProductShareVariables->SetAlarm(11033);
			...
			else
			{
				...
				if (smProductSetting->EnablePickupHeadRetryPickingNo == true)
				{
					...
					{
						if (smProductProduction->nCurrentPickAndPlace1PickingRetry < smProductSetting->PickupHeadRetryPickingNo)
						{
							m_cProductShareVariables->SetAlarm(11014);
							smProductProduction->nCurrentPickAndPlace1PickingRetry++;
							nSequenceNo = nCase.StartMovePickAndPlaceZAxisToDownPlacePositionToPick;
						}
						else
						{
							m_cProductShareVariables->SetAlarm(11031);
				...
				{
					m_cProductShareVariables->SetAlarm(11032);
					smProductEvent->RPNP_RSEQ_BYPASS_PICK_FAIL_REJECT.Set = true;
					smProductProduction->OutputTableResult[0].OutputResult_Post = 0; 
					if (smProductProduction->nCurrentProcessRejectTrayNo == 5)
					{
		...
		case nCase.SetEventPickAndPlacePickProcessDoneAfterPick:
			//smProductProduction->PickAndPlacePickUpHeadStationResult[0] = smProductProduction->OutputTableResult[0];
			if (smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == true)
			{
				smProductProduction->OutputTableResult[0].OutputResult_Post = 0;
				smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 1;
		...
		case nCase.IsTrayTableReadyToPick:
			...
			else if (smProductEvent->RINT_RSEQ_INPUT_UNIT_PICK_START.Set == true && smProductEvent->RMAIN_RTHD_OUTPUT_OR_REJECT_FULL.Set == false)
			{
				if (lnPnPSequenceClockStartCycle.QuadPart != 0)
				{
					...
					smProductProduction->nTime[9] = (int)(lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
					...
					{
						smProductProduction->nTime[0] = smProductProduction->nTime[9];
						m_cLogger->WriteLog("PickAndPlaceSeq%d: UPH cycle time=%lfms.\n", nHeadNo, (double)smProductProduction->nTime[0]);
					}
				}
		...
		case nCase.StartMovePickAndPlaceThetaAxisToDownPosition:
			...
			//smProductProduction->PickAndPlace1ThetaAxisMovePosition = smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition + smProductProduction->InputTableResult[0].InputThetaOffset_mDegree;// + smProductSetting->UnitPlacementRotationOffsetInput;
			smProductProduction->PickAndPlace1ThetaAxisMovePosition = smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition - smProductProduction->InputTableResult[0].InputThetaOffset_mDegree;// + smProductSetting->UnitPlacementRotationOffsetInput;
			smProductProduction->dbCurrentOffset1X = 0;
			smProductProduction->dbCurrentOffset1Y = 0;
			smProductProduction->dbPreviousOffset1X = 0;
			smProductProduction->dbPreviousOffset1Y = 0;
			
			smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um = 0;

			GetNewXYOffsetFromThetaCorretion(smProductProduction->PickAndPlace1ThetaAxisMovePosition - smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition,
				nHeadNo, &smProductProduction->dbCurrentOffset1X, &smProductProduction->dbCurrentOffset1Y, false, false);

			smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisInputPosition - (signed long)(smProductProduction->dbCurrentOffset1X);
			smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisInputPosition - (signed long)(smProductProduction->dbCurrentOffset1Y);
		...
		case nCase.IsMovePickAndPlaceThetaAxisToDownPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ThetaAxisMotorMoveDone.Set == true
				&& smProductEvent->PickAndPlace1XAxisMotorMoveDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveDone.Set == true)
				//&& (smProductEvent->PickAndPlace1ThetaAxisMotorMoveDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.IsStopMovePickAndPlaceThetaAxisToDownPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ThetaAxisMotorStopDone.Set == true)
				//&& (smProductEvent->PickAndPlace1ThetaAxisMotorStopDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.IsMovePickAndPlaceZAxisToDownPositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorMoveToInputTraySoftlandingPositionDone.Set == true)
				//&& (smProductEvent->PickAndPlace1ThetaAxisMotorMoveDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
		...
		case nCase.StartMovePickAndPlaceZAxisToUpAndThetaToOrientationPosition:
			...
			smProductProduction->PickAndPlace1ZAxisMovePosition = smProductTeachPoint->PickAndPlace1ZAxisUpPosition + (signed long)smProductSetting->UnitThickness_um;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0])
			{
				//smProductEvent->PickAndPlace1ZAxisMotorMoveUpPositionDone.Set = false;
				//smProductEvent->StartPickAndPlace1ZAxisMotorMoveUpPosition.Set = true;
		...
		case nCase.IsMovePickAndPlaceZAxisToUpAndThetaToOrientationPositionDone:
			...
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorMoveUpPositionAndRotateDone.Set == true
		...
		case nCase.IsPickAndPlacePickUnitAtUpPositionDone:
			if (smProductEvent->PickAndPlace1ZAxisMotorMoveToInputTraySoftlandingPositionDone.Set == true || smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == true)
			{
				...
				smProductProduction->PickAndPlacePickUpHeadStationResult[0] = smProductProduction->InputTableResult[0];
				GetInputStationResult(nHeadNo);
				ResetInputStationResult();
				m_cProductMotorControl->THKReadPressureValue(0, &dblReturnValue);
				m_cProductMotorControl->THKReadEncoderValue(0, 0, &lngEncoderPositionZ);
				m_cLogger->WriteLog("PickAndPlaceSeq%d: PickAndPlace Pick Z Axis At Up Position Done After Pick.\n", nHeadNo);
				if ((lngEncoderPositionZ > smProductProduction->PickAndPlace1ZAxisMovePosition + 5 || lngEncoderPositionZ < smProductProduction->PickAndPlace1ZAxisMovePosition - 5) && smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == false)
				{
					m_cProductShareVariables->SetAlarm(11035);
				...
				if (dblReturnValue > smProductSetting->PickUpHead1Pressure && smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == false)
				{
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 0;
					...
					if (smProductSetting->EnablePickupHeadRetryPickingNo == true)
					{
						//if (smProductSetting->PickupHeadRetryPickingNo != 0)
						{
							if (smProductProduction->nCurrentPickAndPlace1PickingRetry < smProductSetting->PickupHeadRetryPickingNo)
							{
								//m_cProductShareVariables->SetAlarm(11014);
								smProductProduction->nCurrentPickAndPlace1PickingRetry++;
								m_cLogger->WriteLog("PickAndPlaceSeq%d: Retry Picking.", nHeadNo);
								nSequenceNo = nCase.StartMovePickAndPlaceThetaAxisToDownPosition;
								break;
							}
							else
							{
								//m_cProductShareVariables->SetAlarm(11017);
								smProductProduction->nCurrentInputLotQuantityRun++;
								smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 0;
								smProductEvent->RPNP1_RSEQ_BYPASS_PICK_FAIL.Set = true;
								...
								m_cLogger->WriteLog("PickAndPlaceSeq%d: Skip Unit.", nHeadNo);
								nSequenceNo = nCase.SetEventPickAndPlacePickProcessDone;
								break;
							}
						}
				...
				else
				{
					//smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 1;
					//smProductProduction->PickAndPlacePickUpHeadStationResult[0] = smProductProduction->InputTableResult[0];
					smProductProduction->UpdateMappingProgressHead = 0;
			...
			else
			{
				...
				{
					//m_cProductShareVariables->SetAlarm(11026);
					smProductProduction->nCurrentInputLotQuantityRun++;
					smProductProduction->PickAndPlacePickUpHeadStationResult[0] = smProductProduction->InputTableResult[0];
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 0;
					smProductEvent->RPNP1_RSEQ_BYPASS_PICK_FAIL.Set = true;
		...
		case nCase.SetEventPickAndPlacePickProcessDone:
			...
			smProductEvent->RPNP1_ROUT_THREAD_AWAY_INPUT.Set = true;
			m_cLogger->WriteLog("PickAndPlaceSeq%d: PickAndPlace Pick Process Done.\n", nHeadNo);
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToBottomAndS1Position:
			if (smProductEvent->RSEQ_RPNP1_MOVE_TO_BOTTOM_STATION_START.Set == true)
			{
				smProductEvent->RSEQ_RPNP1_MOVE_TO_BOTTOM_STATION_START.Set = false;
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Receive Event Start PickAndPlace To Bottom Position.\n", nHeadNo);
				if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputTrayNo != smProductProduction->nCurrentInputTrayNumberAtBottom)
				{
					smProductProduction->nCurrentInputTrayNumberAtBottom = smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputTrayNo;
		...
		case nCase.IsBottomS1VisionReady:
			...
			if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1 || smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == true)
			{
			...
			bS1VisionSendRowColumnDone = true;
		...
		case nCase.StartMovePickAndPlaceXYAxisToBottomPosition:
		{
			...
			{
				nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 1000000, 500, 10000000, 10000000); //700000--> 350000 27Nov2024 For Testing Puspose  // back to 700000
				//nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 500000, 500, 8500000, 8500000);
				//nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 50000, 500, 250000, 250000);
			}
			...
			smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisAtS1AndBottomVisionPosition;
				//+ signed long((smProductSetting->PickUpHeadRotationYOffset[0]));

			slBottomPickAndPlaceXAxisMove = smProductProduction->PickAndPlace1XAxisMovePosition;
			slBottomPickAndPlaceYAxisMove = smProductProduction->PickAndPlace1YAxisMovePosition;

			smProductProduction->PickAndPlace1ZAxisMovePosition = smProductTeachPoint->PickAndPlace1ZAxisUpPosition1 + (signed long)smProductSetting->UnitThickness_um;

			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductCustomize->EnablePickAndPlace1XAxisMotor 
				&& smProductCustomize->EnablePickAndPlace1YAxisMotor 
				&& smProductSetting->EnablePH[0])
			{
				smProductEvent->PickAndPlace1XAxisMotorMoveDone.Set = false;
				smProductEvent->StartPickAndPlace1XAxisMotorMove.Set = true;
				smProductEvent->PickAndPlace1YAxisMotorMoveDone.Set = false;
		...
		case nCase.IsMovePickAndPlaceXYZThetaAxisToBottomPositionDone:
			...
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0]
				&& (smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductCustomize->EnablePickAndPlace1YAxisMotor 
					&& smProductEvent->PickAndPlace1XAxisMotorMoveDone.Set == true && smProductEvent->PickAndPlace1YAxisMotorMoveDone.Set == true
					|| smProductCustomize->EnablePickAndPlace1XAxisMotor == false || smProductCustomize->EnablePickAndPlace1YAxisMotor == false)
				&& (smProductEvent->PickAndPlace1ZAxisMotorMoveUpPositionAndRotateDone.Set == true)
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				if ((double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) < smProductProduction->PickAndPlace1XAxisMovePosition - 200
					|| (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) > smProductProduction->PickAndPlace1XAxisMovePosition + 200)
				{
					m_cLogger->WriteLog("PickAndPlace1XAxisCurrentEncoderPosition when bottom not going position %lf.\n", (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)));
					m_cLogger->WriteLog("PickAndPlace1XAxisMovePosition when bottom not going position %lf.\n", (double)(smProductProduction->PickAndPlace1XAxisMovePosition));
					nSequenceNo = nCase.StartMovePickAndPlaceXYAxisToBottomPosition;
					RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
					break;
				}
				...
				smProductProduction->PickAndPlace1StationToMove = PickAndPlaceCurrentStation.UnknownStation;
				smProductEvent->RPNP1_RSEQ_MOVE_TO_BOTTOM_STATION_DONE.Set = true;
				if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 0)
				{
					double dblReturnValue;
					m_cProductMotorControl->THKReadPressureValue(0, &dblReturnValue);
					if (dblReturnValue <= smProductSetting->PickUpHead1Pressure)
					{
						if(smProductEvent->RPNP1_RSEQ_BYPASS_PICK_FAIL.Set == false)
						{
							smProductEvent->RPNP1_RSEQ_BYPASS_PICK_FAIL.Set = true;
					...
					{
						if (smProductEvent->RPNP1_RSEQ_BYPASS_PICK_FAIL.Set == true)
						{
				...
				else if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1 && smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set==false)
				{
					double dblReturnValue;
					m_cProductMotorControl->THKReadPressureValue(0, &dblReturnValue);
		...
		case nCase.StartBottomVisionSOV:
			...
			if (smProductSetting->EnableBottomVision == true && smProductCustomize->EnableBottomVisionModule == true && smProductSetting->EnableVision == true && smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1)
			{
				//smProductEvent->GMAIN_RBTMV_GET_VISION_RESULT_DONE.Set = false;
		...
		case nCase.IsBottomVisionDone:
				...
				|| (smProductSetting->EnableVision == false || smProductCustomize->EnableBottomVisionModule == false || smProductSetting->EnableBottomVision == false))
			{
				...
				{
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um = 0;
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um = 0;
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree = 0;
				}
				if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um > 500 || smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um < -500)
				{
				...
				if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um > 500 || smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um < -500)
				{
				...
				//if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree > 15000 || smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree < -15000)
				//{
				//	m_cProductShareVariables->SetAlarm(6910);
				//	smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree = 0;
				//	smProductProduction->CurrentBottomVisionLoopNo = -1;
				//	nSequenceNo = nCase.StartBottomVisionSOV;
				//}
				...
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Bottom Vision X Offset %lf.\n", nHeadNo, smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um);
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Bottom Vision Y Offset %lf.\n", nHeadNo, smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um);
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Bottom Vision Theta Offset %lf.\n", nHeadNo, smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree);
				//nSequenceNo = nCase.IsRequireAdditionalMoveAndSnap;
				smProductProduction->PickAndPlace1XAxisMovePosition_BeforeMove = smProductProduction->PickAndPlace1XAxisMovePosition;
				smProductProduction->PickAndPlace1YAxisMovePosition_BeforeMove = smProductProduction->PickAndPlace1YAxisMovePosition;
		...
		case nCase.StartMoveOffsetByBottomResult:
			...
			smProductProduction->dbPreviousOffset1X = 0;
			smProductProduction->dbPreviousOffset1Y = 0;
			smProductProduction->dbCurrentOffset1Theta = 0;
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT INPUT ROW %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputRow);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT INPUT COLUMN %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputColumn);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision X Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision Y Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision Theta Offset %lf.\n", nHeadNo, smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE X POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1XAxisMovePosition);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE Y POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1YAxisMovePosition);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE T POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1ThetaAxisMovePosition);

			smProductProduction->PickAndPlace1ThetaAxisMovePosition = smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition 
				- (signed long)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree + (signed long)(smProductSetting->SetupVisionUnitThetaOffset);
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: Lookup Table X %lf.\n", (double)(smProductProduction->dbCurrentOffset1X));
			m_cLogger->WriteLog("PickAndPlaceSeq%d: Lookup Table Y %lf.\n", (double)(smProductProduction->dbCurrentOffset1Y));
			m_cLogger->WriteLog("PickAndPlace1XAxisMovePosition %lf.\n", (double)(smProductProduction->PickAndPlace1XAxisMovePosition));
			m_cLogger->WriteLog("PickAndPlace1YAxisMovePosition %lf.\n", (double)(smProductProduction->PickAndPlace1YAxisMovePosition));
			m_cLogger->WriteLog("PickAndPlace1ThetaAxisMovePosition %lf.\n", (double)(smProductProduction->PickAndPlace1ThetaAxisMovePosition));
		...
		case nCase.IsMoveOffsetByBottomResultDone:
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				smProductProduction->dbPreviousOffset1X = smProductProduction->dbCurrentOffset1X;
				smProductProduction->dbPreviousOffset1X = smProductProduction->dbCurrentOffset1X;
			...
			else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->MOTION_TIMEOUT)
			{
				...
				smProductProduction->PickAndPlace1XAxisMovePosition = smProductProduction->PickAndPlace1XAxisMovePosition_BeforeMove;
				smProductProduction->PickAndPlace1YAxisMovePosition = smProductProduction->PickAndPlace1YAxisMovePosition_BeforeMove;
		...
		case nCase.StartMoveOffsetBySetupOffset:
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision X Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision Y Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision Theta Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomThetaOffset_mDegree);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE X POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1XAxisMovePosition);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE Y POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1YAxisMovePosition);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE T POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1ThetaAxisMovePosition);

			smProductProduction->PickAndPlace1ThetaAxisMovePosition += (signed long)(smProductSetting->SetupVisionUnitThetaOffset);
		...
		case nCase.StartSendSetupRC:
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: Start SEND Setup RC.\n", nHeadNo);
			break;

		case nCase.IsSendSetupRCDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1 || smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 0 || smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == true)
			{
				if ((smProductEvent->GMAIN_RTHD_SETUP_VISION_GET_RC_DONE.Set == true && smProductSetting->EnableS1Vision == true && smProductCustomize->EnableS1VisionModule == true && smProductSetting->EnableVision == true)
					|| (smProductSetting->EnableVision == false || smProductCustomize->EnableS1VisionModule == false || smProductSetting->EnableS1Vision == false))
				{
					m_cLogger->WriteLog("PickAndPlaceSeq%d: Setup Vision get RC Done %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
				...
				else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->MOTION_TIMEOUT)
				{
					...
					else
					{
						...
						RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
						m_cLogger->WriteLog("PickAndPlaceSeq%d: S1 Setup Vision get RC timeout.\n", nHeadNo);
					}					
				}
			}
			/*else
		...
		case nCase.StartSideWall1InspectionGetUnitHigh:
			if (smProductSetting->EnableS1Vision == true && smProductCustomize->EnableS1VisionModule == true && smProductSetting->EnableVision == true /*&& smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1*/)
			{
		...
		case nCase.IsSideWall1InspectionGetUnitHighDone:
				...
				|| (smProductSetting->EnableVision == false || smProductCustomize->EnableS1VisionModule == false || smProductSetting->EnableS1Vision == false))
			{
				...
				if (smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == true)
				{
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].SetupZOffset_um = 0;
					smProductProduction->PickAndPlacePickUpHeadStationResult[0].SetupThetaOffset_mDegree = 0;
				...
				m_cLogger->WriteLog("SetupZOffset_um %lf.\n", (double)(smProductProduction->PickAndPlacePickUpHeadStationResult[0].SetupZOffset_um));
				m_cLogger->WriteLog("SetupThetaOffset_mDegree %lf.\n", (double)(smProductProduction->PickAndPlacePickUpHeadStationResult[0].SetupThetaOffset_mDegree));
		...
		case nCase.StartMovePickAndPlaceThetaAxisForS1Position:
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)THETA START ROTATE TO SETUP OFFSET ANGLE %lf + S1 THETA POS  %lf\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].SetupThetaOffset_mDegree, (double)smProductSetting->S1VisionUnitThetaOffset);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT INPUT ROW %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputRow);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT INPUT COLUMN %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputColumn);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision X Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)Bottom Vision Y Offset %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um);
			...
			smProductProduction->PickAndPlace1ThetaAxisMovePosition = smProductProduction->PickAndPlace1ThetaAxisMovePosition - (signed long)smProductProduction->PickAndPlacePickUpHeadStationResult[0].SetupThetaOffset_mDegree + (signed long)smProductSetting->S1VisionUnitThetaOffset;
		...
		case nCase.StartSendS1RC:
			...
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart4);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: Start Send S1 RC.\n", nHeadNo);
			break;
		...
		case nCase.IsS1AdditionalRequiredMoveAndSnap:
			...
			if (bIsS1VisionRequireAdditionalMoveAndSnap == false)//complete Input Vision
			{
				...
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
				//if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].bolLastUnitInTray == true || smProductProduction->bolIsLastUnitTo1EndTray == true)
				//{
				//	nSequenceNo = nCase.SetBottomEndTrayAfterProgress;
				//}
				//else
		...
		case nCase.StartMoveS1VisionThetaAxisToAdditionalPosition:
			...
			smProductProduction->PickAndPlace1ThetaAxisMovePosition += (signed long)smProductSetting->S1Vision[smProductProduction->CurrentS1VisionLoopNo].ThetaOffset_mDegree;
		...
		case nCase.IsMoveS1VisionThetaAxisToAdditionalPositionDone:
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				smProductProduction->S1VisionModuleMoveThetaCurrentPosition = smProductProduction->PickAndPlace1ThetaAxisMovePosition;
				smProductProduction->dbPreviousOffset1X = smProductProduction->dbCurrentOffset1X;
				smProductProduction->dbPreviousOffset1Y = smProductProduction->dbCurrentOffset1Y;
				nSequenceNo = nCase.IsSendS1RCDone;
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			}
			else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->MOTION_TIMEOUT)
		...
		case nCase.StartS1AdditionalInspectSOV:
			if (smProductSetting->EnableS1Vision == true && smProductCustomize->EnableS1VisionModule == true && smProductSetting->EnableVision == true 
				&& (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1 || smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 0))
			{
				//smProductEvent->GMAIN_RBTMV_GET_VISION_RESULT_DONE.Set = false;
		...
		case nCase.IsS1AdditionalInspectSOVDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((m_cProductIOControl->IsS1VisionGrabDone() == true && smProductSetting->EnableS1Vision == true && smProductCustomize->EnableS1VisionModule == true && smProductSetting->EnableVision == true)
				|| (smProductSetting->EnableVision == false || smProductCustomize->EnableS1VisionModule == false || smProductSetting->EnableS1Vision == false))
			{
				smProductProduction->CurrentS1VisionRetryCount = 0;
				if (smProductEvent->CycleMode.Set == true)
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToS2S3Position:
			if (smProductEvent->RSEQ_RPNP1_MOVE_TO_S3_STATION_START.Set == true)
			{
				smProductEvent->RSEQ_RPNP1_MOVE_TO_S3_STATION_START.Set = false;
				smProductEvent->RMAIN_GMAIN_RESET_S2S3_VISION_RESULT_DONE.Set = false;
				smProductEvent->RMAIN_GMAIN_RESET_S2S3_VISION_RESULT.Set = true;
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Receive Event Start PickAndPlace To S3 Position.\n", nHeadNo);
				if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputTrayNo != smProductProduction->nCurrentInputTrayNumberAtS2S3)
				{
					smProductProduction->nCurrentInputTrayNumberAtS2S3 = smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputTrayNo;
		...
		case nCase.CheckPickAndPlaceModuleSafeToMoveToS2S3Position:
			...
			if (smProductEvent->RMAIN_GMAIN_RESET_S2S3_VISION_RESULT_DONE.Set == true)
			{
				...
				smProductProduction->nCurrentPickupHeadAtS3 = 0;
				smProductProduction->PickAndPlace1XAxisMovePosition_BeforeMove = smProductProduction->PickAndPlace1XAxisMovePosition;
				smProductProduction->PickAndPlace1YAxisMovePosition_BeforeMove = smProductProduction->PickAndPlace1YAxisMovePosition;
				smProductProduction->PickAndPlace1ThetaAxisMovePosition_BeforeMove = smProductProduction->PickAndPlace1ThetaAxisMovePosition;
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			}
			else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->MOTION_TIMEOUT)
			{
		...
		case nCase.StartMovePickAndPlaceXYThetaAxisToS2S3Position:
			...
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT INPUT ROW %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputRow);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT INPUT COLUMN %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].InputColumn);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT X SLEEVE OFFSET %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].SelveXOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)UNIT Y SLEEVE OFFSET %d.\n", nHeadNo, (int)smProductProduction->PickAndPlacePickUpHeadStationResult[0].SelveYOffset_um);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE X POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1XAxisMovePosition);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE Y POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1YAxisMovePosition);
			m_cLogger->WriteLog("PickAndPlaceSeq%d: (-)BEFORE MOVE T POS %lf.\n", nHeadNo, (double)smProductProduction->PickAndPlace1ThetaAxisMovePosition);

			smProductProduction->PickAndPlace1ThetaAxisMovePosition += (signed long)smProductSetting->S2FacetVisionUnitThetaOffset;

			GetNewXYOffsetFromThetaCorretion(smProductProduction->PickAndPlace1ThetaAxisMovePosition - smProductTeachPoint->PickAndPlace1ThetaAxisStandbyPosition,
				nHeadNo, &smProductProduction->dbCurrentOffset1X, &smProductProduction->dbCurrentOffset1Y, true, false);

			smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisAtS2AndS3VisionPosition - (signed long)(smProductProduction->dbCurrentOffset1X);
			smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisAtS2AndS3VisionPosition - (signed long)(smProductProduction->dbCurrentOffset1Y);
		...
		case nCase.IsMovePickAndPlaceXYThetaAxisToS2S3PositionDone:
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				if ((double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) < smProductProduction->PickAndPlace1XAxisMovePosition - 200
					|| (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) > smProductProduction->PickAndPlace1XAxisMovePosition + 200)
				{
					m_cLogger->WriteLog("PickAndPlace1XAxisCurrentEncoderPosition when S2S3 not going position %lf.\n", (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)));
					m_cLogger->WriteLog("PickAndPlace1XAxisMovePosition when S2S3 not going position %lf.\n", (double)(smProductProduction->PickAndPlace1XAxisMovePosition));
					nSequenceNo = nCase.StartMovePickAndPlaceXYThetaAxisToS2S3Position;
					RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
					break;
				}
			...
			{
				//nError = m_cProductMotorControl->AgitoMotorSpeed(0, 0, 50000, 100, 2500000, 2500000);
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Move PickAndPlace Axis To S2 S3 Position timeout %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
		...
		case nCase.IsStopMovePickAndPlaceXYThetaAxisToS2S3PositionDone:
			...
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Stop Move PickAndPlace Theta Axis To S1 Offset Position Done %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
				nSequenceNo = nCase.StartMovePickAndPlaceXYThetaAxisToS2S3Position;//is vision offset position safe to directly move to input station
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			}
			else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->MOTION_TIMEOUT)
			{
		...
		case nCase.StartSendS2FirstSnap:
			...
			//{
				if (smProductSetting->EnableS2Vision == true && smProductCustomize->EnableS2VisionModule == true && smProductSetting->EnableVision == true /*&& smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1*/)
				{
					if (smProductProduction->bPNP2AllowS2S3Snap == true || smProductEvent->GMAIN_RTHD_ENDLOT.Set == true || smProductSetting->EnablePH[1] == false)
					{
						smProductProduction->bPNP2AllowS2S3Snap = false;
		...
		case nCase.IsSendS2FirstSnapProgress:
			...
			if (m_cProductIOControl->IsS2VisionGrabDone() == false)
			{
				if (bPrintLogForFirstTime == false)
				{
					...
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 1.\n");
				}
			...
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 1.\n");
				}				
			}
		...
		case nCase.IsSendS2FirstSnapDone:
			...
			if (m_cProductIOControl->IsS2VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 2.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 2.\n");
				}				
			}
		...
		case nCase.StartSendS3FirstSnap:
			if (smProductSetting->EnableS3Vision == true && smProductCustomize->EnableS3VisionModule == true && smProductSetting->EnableVision == true /*&& smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1*/)
			{
				//smProductEvent->GMAIN_RBTMV_GET_VISION_RESULT_DONE.Set = false;
		...
		case nCase.IsSendS3FirstSnapProgress:
			...
			if (m_cProductIOControl->IsS3VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 3.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 3.\n");
				}				
			}
		...
		case nCase.IsSendS3FirstSnapDone:
			...
			if (m_cProductIOControl->IsS3VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 4.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 4.\n");
				}				
			}
		...
		case nCase.IsS2S3AdditionalRequiredMoveAndSnap:
			...
			{
				m_cLogger->WriteLog("PickAndPlaceSeq: S2 Vision Additional Move And Snap Done.\n");
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
			...
			{
				m_cLogger->WriteLog("PickAndPlaceSeq: S2 Vision Additional Move And Snap.\n");
				RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
				smProductProduction->nCurrentPickupHeadAtS3 = 0;
				nSequenceNo = nCase.StartMovePickAndPlaceThetaAxisToNextPosition;
			}
		...
		case nCase.StartMovePickAndPlaceThetaAxisToNextPosition:
			...
			smProductProduction->PickAndPlace1ThetaAxisMovePosition += (signed long)smProductSetting->S2FacetVision[smProductProduction->CurrentS2VisionLoopNo].ThetaOffset_mDegree;
		...
		case nCase.StartS2AdditionalInspectSOV:
			if (smProductSetting->EnableS2Vision == true && smProductCustomize->EnableS2VisionModule == true 
				&& smProductSetting->EnableVision == true /*&& smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1*/)
			{
				//smProductEvent->GMAIN_RBTMV_GET_VISION_RESULT_DONE.Set = false;
		...
		case nCase.IsS2AdditionalInspectSOVProgress:
			...
			if (m_cProductIOControl->IsS2VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 5.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 5.\n");
				}				
			}
			if ((smProductSetting->EnableS2S3BothSnapping == false && m_cProductIOControl->IsS2VisionGrabDone() == false) 
				|| (smProductSetting->EnableS2S3BothSnapping == true && m_cProductIOControl->IsS2VisionGrabDone() == false && m_cProductIOControl->IsS3VisionGrabDone() == false) 
		...
		case nCase.IsS2AdditionalInspectSOVDone:
			...
			if (m_cProductIOControl->IsS2VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 6.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 6.\n");
				}				
			}
			if ((
				(
				...
				|| (smProductSetting->EnableVision == false || smProductCustomize->EnableS2VisionModule == false || smProductSetting->EnableS2Vision == false))
			{
				...
				m_cProductIOControl->SetS2VisionSOV(false);
				m_cProductIOControl->SetS3VisionSOV(false);
				m_cLogger->WriteLog("PickAndPlaceSeq%d: S2 Vision Facet Done %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
		...
		case nCase.IsS3AdditionalInspectSOVProgress:
			...
			if (m_cProductIOControl->IsS3VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 7.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 7.\n");
				}				
			}
		...
		case nCase.IsS3AdditionalInspectSOVDone:
			...
			if (m_cProductIOControl->IsS3VisionGrabDone() == false)
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: false Shanyu 8.\n");
				}				
			}
			else
			{
				...
				{
					bPrintLogForFirstTime = true;
					m_cLogger->WriteLog("[I/O]: IsInputVisionGrabDone: true Shanyu 8.\n");
				}				
			}
			if (((m_cProductIOControl->IsS3VisionGrabDone() == true) //|| lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount >= 50) 
				&& smProductSetting->EnableS3Vision == true && smProductCustomize->EnableS3VisionModule == true && smProductSetting->EnableVision == true)
		...
		case nCase.IsS2S3VisionAdditionalEOVDone:
			...
			else if (lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount > m_cProductShareVariables->VISION_TIMEOUT)
			{
				if (smProductProduction->nCurrentS2FacetVisionRetryCount < smProductSetting->S2FacetVisionRetryCountAfterFail)
				{
					...
					smProductProduction->PickAndPlace1XAxisMovePosition = smProductProduction->PickAndPlace1XAxisMovePosition_BeforeMove;
					smProductProduction->PickAndPlace1YAxisMovePosition = smProductProduction->PickAndPlace1YAxisMovePosition_BeforeMove;
					smProductProduction->PickAndPlace1ThetaAxisMovePosition = smProductProduction->PickAndPlace1ThetaAxisMovePosition_BeforeMove;
					smProductEvent->RMAIN_GMAIN_RESET_S2S3_VISION_RESULT_DONE.Set = false;
					smProductEvent->RMAIN_GMAIN_RESET_S2S3_VISION_RESULT.Set = true;
					nSequenceNo = nCase.CheckPickAndPlaceModuleSafeToMoveToS2S3Position;
		...
		case nCase.WaitingToReceiveEventStartPickAndPlaceToOutputPosition:
			if (smProductEvent->RSEQ_RPNP1_MOVE_TO_OUTPUT_STATION_START.Set == true)
			{
				smProductProduction->nCurrentPickupHeadAtOutput = 0;
				smProductEvent->RSEQ_RPNP1_MOVE_TO_OUTPUT_STATION_START.Set = false;
		...
		case nCase.StartMovePickAndPlaceXYZThetaAxisToOutputPosition:
		{
			...
			smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisOutputPosition - smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomXOffset_um;

			smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisOutputPosition - smProductProduction->PickAndPlacePickUpHeadStationResult[0].BottomYOffset_um;
		...
		case nCase.IsMovePickAndPlaceXYZThetaAxisToOutputPositionDone:
				...
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
				...
				if ((double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) < smProductProduction->PickAndPlace1XAxisMovePosition - 200
					|| (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)) > smProductProduction->PickAndPlace1XAxisMovePosition + 200)
				{
					m_cLogger->WriteLog("PickAndPlace1XAxisCurrentEncoderPosition when Output not going position %lf.\n", (double)(m_cProductMotorControl->ReadAgitoMotorPosition(0, 0, 0)));
					m_cLogger->WriteLog("PickAndPlace1XAxisMovePosition when Output not going position %lf.\n", (double)(smProductProduction->PickAndPlace1XAxisMovePosition));
					nSequenceNo = nCase.StartMovePickAndPlaceXYZThetaAxisToOutputPosition;
					RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
					break;
				}
				...
				smProductProduction->PickAndPlace1StationToMove = PickAndPlaceCurrentStation.UnknownStation;
				smProductEvent->RPNP1_RSEQ_MOVE_TO_OUTPUT_STATION_DONE.Set = true;
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Move PickAndPlace XYXTheta Axis To Output Position Done %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
		...
		case nCase.StopMovePickAndPlaceXYZThetaAxisToOutputPosition:
			...
			smProductEvent->PickAndPlace1ThetaAxisMotorStopDone.Set = false;
			smProductEvent->StartPickAndPlace1ThetaAxisMotorStop.Set = true;
			m_cLogger->WriteLog("PickAndPlaceSeq%d: Stop Move PickAndPlace XYTheta Axis To Output Position.\n", nHeadNo);
		...
		case nCase.IsTrayTableReadyToPlace:
			if (smProductEvent->ROUT_RSEQ_OUTPUT_UNIT_READY_TO_BE_PLACED.Set == true)
			{
				...
				//	break;
				//}
				if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1 && smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == false)
				{
					double dblReturnValue;
					m_cProductMotorControl->THKReadPressureValue(0, &dblReturnValue);
				...
				else if (smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent == 1)
				{
					smProductProduction->nCurrentPickAndPlace1PlacingRetry = 0;
		...
		case nCase.StartMovePickAndPlaceZAxisToDownPlacePosition:
		{
			...
			if (smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0])
			{
				...
				smProductEvent->StartPickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPosition.Set = true;
			}
		...
		case nCase.IsMovePickAndPlaceZAxisToDownPlacePositionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1Module == true && smProductSetting->EnablePH[0] && smProductEvent->PickAndPlace1ZAxisMotorMoveToOutputTraySoftlandingPositionDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductSetting->EnablePH[0] == false)
			{
			...
			{
				m_cProductShareVariables->SetAlarm(11030);
		...
		case nCase.IsPickAndPlacePickZAxisAtUpPositionDone:
		{
			...
			if ((lngEncoderPositionZ > (smProductProduction->PickAndPlace1ZAxisMovePosition + 5)
				|| lngEncoderPositionZ < (smProductProduction->PickAndPlace1ZAxisMovePosition - 5)) && smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == false)
			{
				m_cProductShareVariables->SetAlarm(11035);
				m_cLogger->WriteLog("PickAndPlaceSeq%d: PickAndPlace Not At Up Position.\n", nHeadNo);
				nSequenceNo = nCase.StartMovePickAndPlaceZAxisToUpPosition;
			...
			if (dblReturnValue > smProductSetting->PickUpHead1Pressure || smProductEvent->GGUI_RSEQ_DRY_RUN_MODE.Set == true)
			{
				//smProductProduction->OutputTableResult[0] = smProductProduction->PickAndPlacePickUpHeadStationResult[0];
				smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 0;
				smProductProduction->PickUpHeadCount[0]++;
				if (smProductProduction->nCurrentProcessRejectTrayNo == 5)
				{
					smProductProduction->nCurrrentTotalUnitDoneByLot++;
					smProductProduction->nCurrentTotalUnitDone++;
			...
			{
				//m_cProductShareVariables->SetAlarm(11015);
				smProductProduction->PickAndPlacePickUpHeadStationResult[0].UnitPresent = 1;
				...
				if (smProductSetting->EnablePickupHeadRetryPlacingNo == true)
				{
					//if (smProductSetting->PickupHeadRetryPickingNo != 0)
					{
						if (smProductProduction->nCurrentPickAndPlace1PlacingRetry < smProductSetting->PickupHeadRetryPlacingNo)
						{
							m_cProductShareVariables->SetAlarm(11015);
							smProductProduction->nCurrentPickAndPlace1PlacingRetry++;
							nSequenceNo = nCase.StartMovePickAndPlaceZAxisToDownPlacePosition;
						}
						else
						{
							m_cProductShareVariables->SetAlarm(11028);
				...
				{
					m_cProductShareVariables->SetAlarm(11029);
					nSequenceNo = nCase.IsUnitRemovedAfterPlaceFail;
					break;
				}
				//RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockStart2);
		...
		case nCase.SetEventPickAndPlacePlaceProcessDone:
			...
			if (smProductEvent->RPNP1_RSEQ_Y_MOVE_STANDBY.Set == true)
			{
				smProductEvent->RPNP1_RSEQ_Y_MOVE_STANDBY.Set = false;
		...
		case nCase.IsEventPickAndPlaceDisable:
			if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation)
			{
			...
			else if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.InputStation)
			{
		...
		case nCase.StartMovePickAndPlaceYAxisToParkingPosition2:
			if (smProductCustomize->EnablePickAndPlace1YAxisMotor)// && smProductSetting->EnablePH[0])
			{
				smProductProduction->PickAndPlace1XAxisMovePosition = smProductTeachPoint->PickAndPlace1XAxisParkingPosition;
				smProductProduction->PickAndPlace1YAxisMovePosition = smProductTeachPoint->PickAndPlace1YAxisStandbyPosition;
				//smProductEvent->PickAndPlace1XAxisMotorMoveToParkingPositionDone.Set = false;
		...
		case nCase.IsMovePickAndPlaceYAxisToParkingPositionDone2:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1YAxisMotor && smProductEvent->PickAndPlace1YAxisMotorMoveToStandbyPositionDone.Set == true && smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation)// && smProductSetting->EnablePH[0]
		...
		case nCase.StartMovePickAndPlaceXAxisToParkingPosition2:
			if (smProductCustomize->EnablePickAndPlace1XAxisMotor)// && smProductSetting->EnablePH[0])
			{
				smProductEvent->PickAndPlace1XAxisMotorMoveToParkingPositionDone.Set = false;
				smProductEvent->StartPickAndPlace1XAxisMotorMoveToParkingPosition.Set = true;
		...
		case nCase.IsMovePickAndPlaceXAxisToParkingPositionDone2:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductCustomize->EnablePickAndPlace1XAxisMotor && smProductEvent->PickAndPlace1XAxisMotorMoveToParkingPositionDone.Set == true && smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation)
				|| smProductCustomize->EnablePickAndPlace1XAxisMotor == false)
			{
				smProductProduction->PickAndPlace1CurrentStation = PickAndPlaceCurrentStation.DisableStation;
				smProductProduction->PickAndPlace1StationToMove = PickAndPlaceCurrentStation.UnknownStation;
		...
		case nCase.IsMovePickAndPlaceXAxisToStandbyStationPostProductionDone:
			RtGetClockTime(CLOCK_FASTEST, &lnPnPSequenceClockEnd);
			lnPnPSequenceClockSpan.QuadPart = lnPnPSequenceClockEnd.QuadPart - lnPnPSequenceClockStart2.QuadPart;
			if ((smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.DisableStation && smProductEvent->PickAndPlace1XAxisMotorMoveToParkingPositionDone.Set == true)
				|| smProductCustomize->EnablePickAndPlace1Module == false || smProductCustomize->EnablePickAndPlace1XAxisMotor == false)
			{
				m_cLogger->WriteLog("PickAndPlaceSeq%d: Move PickAndPlace X Axis To Standby Station Post Production Done %ums.\n", nHeadNo, lnPnPSequenceClockSpan.QuadPart / m_cProductShareVariables->m_TimeCount);
				smProductEvent->RPNP1_RSEQ_POST_PRODUCTION_DONE.Set = true;
				//if (smProductProduction->PickAndPlace1StationToMove == PickAndPlaceCurrentStation.BottomStation)
				//{
				//	smProductProduction->PickAndPlace1CurrentStation = PickAndPlaceCurrentStation.BottomStation;
				//	smProductProduction->PickAndPlace1StationToMove = PickAndPlaceCurrentStation.UnknownStation;
