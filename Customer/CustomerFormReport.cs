...
namespace Customer
{
    public class CustomerFormReport : Product.ProductFormReport
    {
        ...
        private TimeSpan m_tsLoadingTime = new TimeSpan();
        private TimeSpan m_tsUnloadingTime = new TimeSpan();
        private TimeSpan m_tsCleaningTime = new TimeSpan();
        private TimeSpan m_tsRejectTrayExchangeTime = new TimeSpan();
        private double m_dPercentageLoadingTime = 0;
        private double m_dPercentageUnloadingTime = 0;
        private double m_dPercentageCleaningTime = 0;
        private double m_dPercentageRejectTrayExchangeTime = 0;
        ...
        override public void InitializeAlarmList()
        {
            try
            {
                ...
                CustomerAlarmList alarmList = new CustomerAlarmList();
        ...
        override public int GetUpDownSetting(int eventID, ReportEvent reportEvent)
        {
            ...
            #region Customize
            //Customize
            if (eventID == m_CustomerReportEvent.EventLoadMaterialTime.EventID)
            {
                nSetting = m_CustomerReportEvent.EventLoadMaterialTime.EventResult;
            }
            else if (eventID == m_CustomerReportEvent.EventUnloadMaterialTime.EventID)
            {
                nSetting = m_CustomerReportEvent.EventUnloadMaterialTime.EventResult;
            }
            else if (eventID == m_CustomerReportEvent.EventRejectTrayExchangeTime.EventID)
            {
                nSetting = m_CustomerReportEvent.EventRejectTrayExchangeTime.EventResult;
            }
            else if (eventID == m_CustomerReportEvent.EventCleaningTime.EventID)
            {
                nSetting = m_CustomerReportEvent.EventCleaningTime.EventResult;
            }
            #endregion
            else
            {
        ...
        override public int CalculatePerformance(ReportProcess.Record[] record, List<int> alarmFilter, ReportEvent reportEvent, out MachinePerformance machinePerformance)
        {
            ...
            m_tsLoadingTime = new TimeSpan();
            m_tsUnloadingTime = new TimeSpan();
            m_tsCleaningTime = new TimeSpan();
            m_tsRejectTrayExchangeTime = new TimeSpan();
            ...
            try
            {
                ...
                foreach (var _reportInfo in record)
                {
                    ...
                    else
                    {
                        ...
                        #region Customize
                        if (recordPrevious.eventID == m_CustomerReportEvent.EventLoadMaterialTime.EventID)  // alarm
                        {
                           // machinePerformance.FailureCount++;
                            m_tsLoadingTime += _reportInfo.dateTime.Subtract(recordPrevious.dateTime);
                        }
                        if (recordPrevious.eventID == m_CustomerReportEvent.EventUnloadMaterialTime.EventID)  // alarm
                        {
                            // machinePerformance.FailureCount++;
                            m_tsUnloadingTime += _reportInfo.dateTime.Subtract(recordPrevious.dateTime);
                        }
                        if (recordPrevious.eventID == m_CustomerReportEvent.EventCleaningTime.EventID)  // alarm
                        {
                            // machinePerformance.FailureCount++;
                            m_tsCleaningTime += _reportInfo.dateTime.Subtract(recordPrevious.dateTime);
                        }
                        if (recordPrevious.eventID == m_CustomerReportEvent.EventRejectTrayExchangeTime.EventID)  // alarm
                        {
                            // machinePerformance.FailureCount++;
                            m_tsRejectTrayExchangeTime += _reportInfo.dateTime.Subtract(recordPrevious.dateTime);
                        }
                        #endregion
                ...
                if (machinePerformance.TotalTime.Ticks != 0)
                {
                    ...
                    #region Customize
                    m_dPercentageLoadingTime = Math.Round(((double)m_tsLoadingTime.Ticks / (double)machinePerformance.TotalTime.Ticks) *100, 2);
                    m_dPercentageUnloadingTime = Math.Round(((double)m_tsUnloadingTime.Ticks / (double)machinePerformance.TotalTime.Ticks) * 100, 2);
                    m_dPercentageCleaningTime = Math.Round(((double)m_tsCleaningTime.Ticks / (double)machinePerformance.TotalTime.Ticks) * 100, 2);
                    m_dPercentageRejectTrayExchangeTime = Math.Round(((double)m_tsRejectTrayExchangeTime.Ticks / (double)machinePerformance.TotalTime.Ticks) * 100, 2);
                    #endregion
                }
                else
                {
                    ...
                    #region Customize
                    m_dPercentageLoadingTime = Math.Round(0.0,2);
                    m_dPercentageUnloadingTime = Math.Round(0.0,2);
                    m_dPercentageCleaningTime = Math.Round(0.0,2);
                    m_dPercentageRejectTrayExchangeTime = Math.Round(0.0, 2);
                    #endregion
                }
        ...
        override public int GenerateReportToListString(DateTime startDate, DateTime endDate, MachinePerformance machinePerformance, ReportProcess.Record[] record, List<AlarmInfo2> listAlarmRecord, List<int> alarmFilter, out StringBuilder data)
        {
            ...
            try
            {
                ...
                #region Customize
                data.Append(string.Format("{0,-18} {1,-1} {2,-15} {3,-20} {4,-15}\r\n", "LOADING TIME", ":", m_tsLoadingTime.ToString(@"dd\.hh\:mm\:ss") + " (" +m_dPercentageLoadingTime + "%)", "", ""));
                data.Append(string.Format("{0,-18} {1,-1} {2,-15} {3,-20} {4,-15}\r\n", "UNLOADING TIME", ":", m_tsUnloadingTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageUnloadingTime + "%)", "", ""));
                //data.Append(string.Format("{0,-18} {1,-1} {2,-15} {3,-20} {4,-15}\r\n", "REJECT TRAY EXCHANGE TIME", ":", m_tsExchangeRejectTrayTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageExchangeRejectTrayTime + "%)", "", ""));
                //data.Append(string.Format("{0,-18} {1,-1} {2,-15} {3,-20} {4,-15}\r\n", "CLEANING TIME", ":", m_tsCleaningTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageCleaningTime + "%)", "", ""));
                #endregion
        ...
        override public int GenerateReportToCSV(DateTime startDate, DateTime endDate, MachinePerformance machinePerformance, ReportProcess.Record[] report, List<AlarmInfo2> listAlarmRecord, List<int> alarmFilter, out StringBuilder data)
        {
            ...
            try
            {
                ...
                #region Customize
                data.AppendLine(string.Format("{0,-18}, {1,-1} {2,-15} {3,-20} {4,-15}", "LOADING TIME", ":", m_tsLoadingTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageLoadingTime + "%)", "", ""));
                data.AppendLine(string.Format("{0,-18}, {1,-1} {2,-15} {3,-20} {4,-15}", "UNLOADING TIME", ":", m_tsUnloadingTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageUnloadingTime + "%)", "", ""));
               // data.AppendLine(string.Format("{0,-18}, {1,-1} {2,-15} {3,-20} {4,-15}", "REJECT TRAY EXCHANGE TIME", ":", m_tsExchangeRejectTrayTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageExchangeRejectTrayTime + "%)", "", ""));
                //data.AppendLine(string.Format("{0,-18}, {1,-1} {2,-15} {3,-20} {4,-15}", "CLEANING TIME", ":", m_tsCleaningTime.ToString(@"dd\.hh\:mm\:ss") + " (" + m_dPercentageCleaningTime + "%)", "", ""));
                #endregion
