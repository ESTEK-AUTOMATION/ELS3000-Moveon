            ...
            try
            {
                ...
                foreach (FieldInfo f in fiOutputSetting)
                {
                    if (f.FieldType.IsGenericType && (f.FieldType.GetGenericTypeDefinition() == typeof(List<>)))
                    {
                        ...
                        if (OutputDefectData_New.Count == OutputDefectData_Ori.Count)
                        {
                        ...
                        else if(OutputDefectData_New.Count > OutputDefectData_Ori.Count)
                        {
                            int NewIndex = 0;
                            for (int i = 0; i < OutputDefectData_Ori.Count; i++)
                            {
                                foreach (FieldInfo field in OutputDefectField)
                                {
                                    if (!object.Equals(field.GetValue(OutputDefectData_New[i]), field.GetValue(OutputDefectData_Ori[i])))
                            ...
                            for (int i = NewIndex; i < OutputDefectData_New.Count; i++)
                            {
                                string NewParameter = $"{DateTime.Now.ToString("yyyyMMdd HHmmss")} Recipe Type: Output Recipe, Recipe Name: {m_tabpageOutput.comboBoxRecipeOutput.SelectedItem.ToString()}, New Parameter: {f.Name}, List {i + 1} ";
                                foreach (FieldInfo field in OutputDefectField)
                                {
                                    NewParameter += $",{field.Name} = {field.GetValue(OutputDefectData_New[i])} ";
                                }
                                Machine.EventLogger.WriteLog(NewParameter);
                                //RecipeParameterLogger.WriteLog(NewParameter);
                            }
                        }
                        else if (OutputDefectData_New.Count < OutputDefectData_Ori.Count)
                        {
                            ...
                            for (int i = OldIndex; i < OutputDefectData_Ori.Count; i++)
                            {
                                string OldParameter = $"{DateTime.Now.ToString("yyyyMMdd HHmmss")} Recipe Type: Output Recipe, Recipe Name: {m_tabpageOutput.comboBoxRecipeOutput.SelectedItem.ToString()}, Parameter Removed: {f.Name}, List {i + 1} ";
                                foreach (FieldInfo field in OutputDefectField)
                                {
                                    OldParameter += $",{field.Name} = {field.GetValue(OutputDefectData_Ori[i])} ";
                                }
                                Machine.EventLogger.WriteLog(OldParameter);
                                //RecipeParameterLogger.WriteLog(OldParameter);
                            }
                        }
                    }
                    else
            ...
            try
            {
                ...
                foreach (FieldInfo f in fiOutputFileSetting)
                {
                    if (f.FieldType.IsGenericType && (f.FieldType.GetGenericTypeDefinition() == typeof(List<>)))
                    {
                        ...
                        if (DefectData_New.Count == DefectData_Ori.Count)
                        {
                        ...
                        else if(DefectData_New.Count>DefectData_Ori.Count)
                        {
                            int NewIndex = 0;
                            for (int i = 0; i < DefectData_Ori.Count; i++)
                            {
                                foreach (FieldInfo field in DefectField)
                                {
                                    if (!object.Equals(field.GetValue(DefectData_New[i]), field.GetValue(DefectData_Ori[i])))
                            ...
                            for(int i = NewIndex;i<DefectData_New.Count;i++)
                            {
                                string NewParameter = $"{DateTime.Now.ToString("yyyyMMdd HHmmss")} Recipe Type: Output File Recipe, Recipe Name: {m_tabpageOutputFile.comboBoxRecipeOutputFile.SelectedItem.ToString()}, New Parameter: {f.Name}, Defect {i + 1} ";
                                foreach (FieldInfo field in DefectField)
                                {
                                    NewParameter += $",{field.Name} = {field.GetValue(DefectData_New[i])} ";
                                }
                                Machine.EventLogger.WriteLog(NewParameter);
                                //RecipeParameterLogger.WriteLog(NewParameter);
                            }
                        ...
                        else if(DefectData_New.Count < DefectData_Ori.Count)
                        {
                            ...
                            for (int i = OldIndex; i < DefectData_Ori.Count; i++)
                            {
                                string OldParameter = $"{DateTime.Now.ToString("yyyyMMdd HHmmss")} Recipe Type: Output File Recipe, Recipe Name: {m_tabpageOutputFile.comboBoxRecipeOutputFile.SelectedItem.ToString()}, Parameter Removed: {f.Name}, Defect {i + 1} ";
                                foreach (FieldInfo field in DefectField)
                                {
                                    OldParameter += $",{field.Name} = {field.GetValue(DefectData_Ori[i])} ";
                                }
                                Machine.EventLogger.WriteLog(OldParameter);
                                //RecipeParameterLogger.WriteLog(OldParameter);
                            }
            ...
            try
            {
                ...
                foreach (FieldInfo f in fiVisionSetting)
                {
                    if (f.FieldType.IsGenericType && (f.FieldType.GetGenericTypeDefinition() == typeof(List<>)))
                    {
                        ...
                        if (VisionData_New.Count == VisionData_Ori.Count)
                        {
                        ...
                        else if(VisionData_New.Count > VisionData_Ori.Count)
                        {
                            int NewIndex = 0;
                            for (int i = 0; i < VisionData_Ori.Count; i++)
                            {
                                foreach (FieldInfo field in VisionField)
                                {
                                    if (!object.Equals(field.GetValue(VisionData_New[i]), field.GetValue(VisionData_Ori[i])))
                            ...
                            for (int i = NewIndex; i < VisionData_New.Count; i++)
                            {
                                string NewParameter = $"{DateTime.Now.ToString("yyyyMMdd HHmmss")} Recipe Type: Vision Recipe, Recipe Name: {m_tabpageVisionRecipe.comboBoxRecipeVision.SelectedItem.ToString()}, New Parameter: {f.Name}, Snap {i + 1} ";
                                foreach (FieldInfo field in VisionField)
                                {
                                    NewParameter += $",{field.Name} = {field.GetValue(VisionData_New[i])} ";
                                }
                                Machine.EventLogger.WriteLog(NewParameter);
                                //RecipeParameterLogger.WriteLog(NewParameter);
                            }
                        ...
                        else if(VisionData_New.Count < VisionData_Ori.Count)
                        {
                            ...
                            for (int i = OldIndex; i < VisionData_Ori.Count; i++)
                            {
                                string OldParameter = $"{DateTime.Now.ToString("yyyyMMdd HHmmss")} Recipe Type: Vision Recipe, Recipe Name: {m_tabpageVisionRecipe.comboBoxRecipeVision.SelectedItem.ToString()}, Parameter Removed: {f.Name}, Snap {i + 1} ";
                                foreach (FieldInfo field in VisionField)
                                {
                                    OldParameter += $",{field.Name} = {field.GetValue(VisionData_Ori[i])} ";
                                }
                                Machine.EventLogger.WriteLog(OldParameter);
                                //RecipeParameterLogger.WriteLog(OldParameter);
                            }
